package buttons 
{
	...
	public class Button extends LayerX
	{
		...
		public function set state(mode:int):void {
			...
			_oldState = this.mode;				// запоминаем предыдущее состояние кнопки.
...
/*			if(this.parent){
		...
		private function onBeforeClick(e:MouseEvent):void 
		{
			...
			if (Quests.lockButtons && !settings.ignoreAll) {
				//trace('stopImmediatePropagation2')
				e.stopImmediatePropagation();
				Quests.lockButtons = false;
			}
			
			if (!App.user.quests.tutorial && App.isSocial('MX','AI','YB')) {
				if (!reask 
					&& this.settings.type != "seaStone" 
					&& this.settings.type != "fertilizer" 
					&& this.settings.type != "eventCoin" 
					&& ((this is MoneySmallButton) || (this is MoneyButton) || (this is Button && (this.settings.hasOwnProperty('diamond'))) || settings.type == "sets" || settings.type == "decorSale" || settings.type == "happy"))  
				{
					reaskWindow();
					e.stopImmediatePropagation();
				} else {
		...
		private function reaskWindow():void {
			var count:int = 0;
			/*if (this is SocialButton) {
				count = (this as SocialButton).settings.price;
			}else */if (this is MoneySmallButton) {
			...
			var that:* = this;
		/*	var win:SimpleWindow = new SimpleWindow({
				hasExit:true,
				width:410,
				height:240,
				buttonText:Locale.__e('flash:1382952380299'),
				title:Locale.__e('flash:1401434896283'),
				text:Locale.__e("flash:1410336571371", [String(count)]),
				popup:true,				
				dialog:true,
		...
		public function fant(params:Object = null):void {
			fantImage = new Bitmap(UserInterface.textures.fantsIcon, 'auto', true);
