package ui 
{
	...
	public class UpPanel extends Sprite
	{
		...
		public function UpPanel() 
		{
			...
			var itemsReward:Array = [];
			for (var sID:* in App.data.storage) {
				var object:Object = App.data.storage[sID];
				object['sid'] = sID;
				if (object.type == 'Reals' || object.type == 'Coins'){
					if (object.socialprice.hasOwnProperty(App.social) && object.hasOwnProperty('reward') && Numbers.firstProp(object.reward).key == optionItem)
						itemsReward.push(object); 
				}
			}
			
			if (App.social == 'FB' && optionItem > 0 && itemsReward.length > 0)
				drawTopReward();
			...
			//var persIcon:PersonageIcon;
			
			//persIcon = new PersonageIcon(Techno.TECHNO);
		...
		{
			if (App.user.mode == User.OWNER && !App.user.quests.tutorial && bonusOnlineBttn && App.user.worldID != User.HOLIDAY_LOCATION)
				bonusOnlineBttn.visible = true;
			
			if (!bonusOnlineBttn && App.user.bonusOnline.showIcon && !App.user.quests.tutorial && App.user.mode != User.GUEST && App.user.worldID != User.AQUA_HERO_LOCATION) 
			{
				...
				var bonusOnlineUpdate:Function = function():void {
					if (App.user.mode != User.OWNER || App.user.quests.tutorial || App.user.worldID == User.HOLIDAY_LOCATION){
		...
		private function onLoadImage(data:Object):void
		{
			...
			guestPlusBttn.bitmapData =data.bitmapData;
				/*if (guestPlusBttn.width>guestBar.width-offset) 
				{
					guestPlusBttn.width = 	guestBar.width - offset;
					guestPlusBttn.scaleY = guestPlusBttn.scaleX;
					
				}*/
				Size.size(guestPlusBttn, guestBar.width - 25, guestBar.width - 25);
				/*if (guestPlusBttn.height>guestBar.height-offset) 
				{
				guestPlusBttn.height = 	guestBar.height -offset;
				guestPlusBttn.scaleX = guestPlusBttn.scaleY;
		...
		//	}*/
			...
			for	(var fId:* in App.ui.bottomPanel.friendsPanel.allFriendsItems)
			{
				if (App.ui.bottomPanel.friendsPanel.allFriendsItems[fId].friend.uid == App.owner.id)
				currentFriend = fId;
			}
			...
			arrowLeft.tip = function():Object {
				return {
					title:App.ui.bottomPanel.friendsPanel.allFriendsItems[(currentFriend - 1 < 0)?App.ui.bottomPanel.friendsPanel.allFriendsItems.length - 1:currentFriend - 1].friend.first_name,
			...
			arrowRight.tip = function():Object {
				return {
					//title:App.ui.bottomPanel.friendsPanel.allFriendsItems[(currentFriend - 1 < 0)?App.ui.bottomPanel.friendsPanel.friendsItems.length - 1:currentFriend - 1].friend.first_name,
					
					title:App.ui.bottomPanel.friendsPanel.allFriendsItems[(currentFriend + 1)>=App.ui.bottomPanel.friendsPanel.allFriendsItems.length?0:currentFriend + 1].friend.first_name,
					text:Locale.__e('flash:1382952379780')
					//App.user.friends
				};
			};
		...
		public function drawGuestWakeUpPanel():void 
		{
			...
			if (!Friends.isOwnerSleeping(App.owner.id)) return;
			...
			var extra:ExtraItem = new ExtraItem( { 7:5, 1:50, 2:30 }, this);
		...
		{			
			...
			if (basketSprite) 
			{
				if (App.user.worldID == User.HOLIDAY_LOCATION && App.user.mode != User.GUEST)
				{
			...
			if (winterShopBttn) 
				{
				if (App.user.quests.tutorial || User.inExpedition || App.user.mode == User.GUEST) 
				{
					winterShopBttn.visible = false;
					timeOfExpire.visible = false;
				}else 
				{
					winterShopBttn.visible = true;
					timeOfExpire.visible = true;
				}
			}*/
			...
			if (ribbonSaleText)
			{
				ribbonSaleText.x = ribbonInterBankSale.x + (ribbonInterBankSale.width - ribbonSaleText.width) / 2;
				
				ribbonSaleText.y = ribbonInterBankSale.y + (ribbonInterBankSale.height - ribbonSaleText.textHeight) / 2 + 7;
				//ribbonSaleText.width += 15;
				timeSaleText.x = ribbonInterBankSale.x + (ribbonInterBankSale.width - timeSaleText.width) / 2;
				timeSaleText.y = ribbonInterBankSale.y + ribbonInterBankSale.height - timeSaleText.height - 10;
			}
		}	
		...
		private function updateCookies():void 
		{
			...
			/*cookieAmount = String(App.user.stock.count(App.data.storage[App.user.worldID].cookie[0]));
			cookieAmount = '1';
			cookiesCounter.text = cookieAmount;
			
			cookiesContainer.hideGlowing();*/
			
			
			/*if (!App.user.quests.tutorial&&(App.user.mode != User.GUEST)&&int(cookieAmount) <= App.data.options.CookiesAlarmLimit) 
			{
				
				cookiesContainer.showGlowing(0xff0000);					
				
				finded = []
				for (var s2:* in App.data.storage) {
					if (App.data.storage[s2].hasOwnProperty('devel')) {
						var crafting2:* = App.data.storage[s2].devel.craft;
						for each(var cftItm2:* in crafting2) {
								for each(var cft2:* in cftItm2) {
								if (App.data.crafting.hasOwnProperty(cft2)&&App.data.crafting[cft2].out == int(App.data.storage[App.user.worldID].cookie[0])) {
								finded.push(s2);
				...
				if (finded) {
					var onMap2:Array = Map.findUnits(finded);
					for (var j:int = 0; j < onMap2.length; j++) 
					{
						if (!onMap2[j].crafting&&!onMap2[j].hasProduct) 
						{
						onMap2[j].showGlowing(0xff9900);
					//	onMap2[j].cloudResource(true, Stock.COOKIE, onMap2[j].click, 'productBacking2', 0.6, false, 0, 0);
