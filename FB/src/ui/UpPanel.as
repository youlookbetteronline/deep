package ui 
{
	...
	public class UpPanel extends Sprite
	{
		...
		public function UpPanel() 
		{
			...
			//Спрайт бодрости
			moxieSprite.x = (App.self.stage.stageWidth - moxieSprite.width) / 2 - 0;/*(App.self.stage.stageWidth - basketSprite.width) / 2 - 198*/;
			moxieSprite.y = 14;
		...
		public function addBankSaleIcon():void
		{
			...
			if (ribbonInterBankSale && ribbonInterBankSale.parent)
				ribbonInterBankSale.parent.removeChild(ribbonInterBankSale);
		...
		public function drawInvitetoMap():void 
		{
			...
			inviteBttn.tip = function():Object {
				return {
					title:Locale.__e("Пригласить друга!"),
					text:Locale.__e("Нажми, чтобы пригласить друга!")
				};
			}
		...
		{			
			...
			if (moxieSprite) 
			{
				...
				if (App.data.storage[App.user.worldID].cookie && App.data.storage[App.user.worldID].cookie[0] != 519 && App.user.mode == User.OWNER)
				{
					...
					moxieSprite.y = 14;
					moxieSprite.x = (App.self.stage.stageWidth - moxieSprite.width) / 2 - 0;
				}else	
