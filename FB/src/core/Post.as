package core
{
	...
	public class Post extends EventDispatcher
	{
		...
		private static function request():void
		{
			...
			requestVars.crc = crc;
			requestVars.pid = pid;
			if (Post.h) requestVars.h = Post.h;
		...
		private static function onComplete(e:Event):void
		{
			...
			if (response.data != null)
			{
				...
				if (response.data.hasOwnProperty('__boostedBonus'))
				{
					delete response.data['__boostedBonus'];
				}
		...
		{
			Post.send({ctr: 'User', act: 'stat', uID: App.user.id, type: type}, function(error:int, data:Object, params:Object):void
			{
				if (error)
				{
		...
		private static function OnIOError(event:IOErrorEvent):void
		{
			...
			{
				new SimpleWindow({title: Locale.__e('flash:1382952379725'), text: Locale.__e('flash:1382952379726'), label: SimpleWindow.ERROR});
			}
		}
