package core 
{
	...
	import units.Unit;
	import wins.UpgradeFinishWindow;
	import wins.Window;
	/**
	 * ...
	 * @author 
	...
	public class WallPost 
	{
		...
		public static function showFinishWindow(sid:int):void 
		{
			if (_buildings.indexOf(sid) == -1) 
				return;
				
			if (App.social != 'FB')	return;
			new UpgradeFinishWindow( { sID:sid, callback:function():void {
					var _imageURL:String = getBuildingImage(sid);
					ExternalApi.apiNormalScreenEvent();
					ExternalApi.addPostCallback(function():void { } );
					App.network.wallPost( {
						type:8,
						owner_id:App.user.id,
						//bytes:pngStream,
						msg:Strings.__e('AUnit_makePost',[App.data.storage[sid].title]),
						hasCallback:false,
						sID:sid,
						url:_imageURL
					});
		...
		private static var _buildings:Array = [486, 183, 305, 306, 307, 308, 487, /*499,*/ 500, /*526, */531, 532, 533, 602, 707, 709, 710, 712, 714, 782, 787, 796, 908, 1165, 1174, 1257, 1258, 1416];
		public static function getBuildingImage(sid:int):String 
		{
			if (_buildings.indexOf(sid) != -1) 
			{
				var _name:String = '486x258_' + String(sid);
				//_name = 'test_4';
				return Config.getImage('wallpost/FB/buildings', _name, 'jpg');
			}
			return Config.getImage('wallpost/FB/buildings', '486x258_526', 'jpg');
		}
