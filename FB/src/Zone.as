package 
{
	...
	public class Zone extends LayerX
	{
		...
		public static var zonePoint:Object = {
			935:{x:53, y:128}, 	//Первая зона
			939:{x:90, y:159}, 	//Первая зона
			934:{x:47, y:157} 	//155 квест
		};
		
		public var fader:Bitmap;
		public var border:Bitmap;
		public var ZoneLayer:LayerX = new LayerX();
		...
		public var container:LayerX = new LayerX();
		private function drawInformer():void
		{
			...
			var titleText:TextField = Window.drawText(App.data.storage[sID].title, textSettings);
				...
				bg = Window.backing(280, titleText.height +50, 20, "newTerritorySign");
				container.addChildAt(bg, 0);
				Window.addMirrowObjs(container, 'diamondsTop', bg.width / 2 +14, bg.width / 2-14, -10, true, true);
				Window.addMirrowObjs(container, 'storageWoodenDec', -20, bg.width + 20, bg.height - 80);
				
				titleText.x = (bg.width - titleText.width)/2 - 42;
		...
		public function glowTheSign():void 
		{
			var targetContainer:* = this.container;
			targetContainer.showGlowing();
			setTimeout(function():void {
				targetContainer.hideGlowing();
			}, 6000);			
		}
		...
		private function drawBorder(array:Array):Bitmap
		{
			var p0:Object = array[0];
			var point0:Object = IsoConvert.isoToScreen(p0.x, p0.z, true);
			...
			border.graphics.moveTo(point0.x - dX, point0.y - dY);
			...
			for (var i:int = 1; i < L; i++)
			{
				p = array[i];
				point = IsoConvert.isoToScreen(p.x, p.z, true);
				border.graphics.lineTo(point.x - dX, point.y - dY);
				
				if (point.x - dX < minX) minX = point.x - dX;
				if (point.y - dY < minY) minY = point.y - dY;
			}
			
			border.graphics.lineTo(point0.x - dX, point0.y - dY);
			border.graphics.endFill();
			
			var bitmap:Bitmap = new Bitmap(snapClip(border));
			bitmap.filters = [ new BlurFilter(4, 4, 1) ];
		...
		public function deleteInformer():void
		{
		...
		public static function snapClip(clip:*, delta:int = 20):BitmapData
		{
