package hlp {
	...
	public class PropsTool {
		...
		private static function addZone():void {
			ToolsUtils.removeFromParent(_zone);
			_zone = HighlightZone.zone(x_coord, z_coord, x_size, z_size, 0x00ff00, false);
		...
		private static function wrk(e:* = null):void
		{
			...
			for (var i:int = x_coord; i < x_coord + x_size; i++)
			{
				for (var j:int = z_coord; j <= z_coord + z_size; j++)
				{
					var node:AStarNodeVO = App.map._aStarNodes[i][j];
					if (node.object && node.object is Resource)
					{
						if (node.object.coords.x >= i /*&& node.object.coords.x + node.object.cells < x_coord + x_size*/ &&
			...
			for each(var _unit:Resource in props) {
				result.push( {
					sid:_unit.sid,
					x:_unit.coords.x - x_coord,// - (x_coord - x_size),
					z:_unit.coords.z - z_coord,// - (z_coord - z_size),
					rotate:_unit.rotate
				})
			}
