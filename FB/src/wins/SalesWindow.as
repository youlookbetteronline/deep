package wins 
{
	...
	//	public var action:Object;
		...
		private function initContent(data:Object):Array
		{
			...
			result.sortOn('order', Array.NUMERIC | Array.DESCENDING);
			return result;
		}
		...
		override public function drawBody():void {
			...
			paginator.pointsLeft.x -= 15;
			paginator.pointsRight.x += 12;
			...
			var descriptionLabel:TextField = drawText(Locale.__e("flash:1428668029579"), {
				fontSize:26,
				autoSize:"left",
				textAlign:"center",
				color:0xffffff,
			...
			drawMirrowObjs('diamonds', -4, settings.width + 4, settings.height - 106);
			drawMirrowObjs('diamonds', -4, settings.width + 4, 17, false, false, false, 1, -1);
			
			paginator.x -= PageButton.WIDTH;
			paginator.y += PageButton.HEIGHT;
		...
		override public function drawArrows():void {
			...
			paginator.arrowLeft.y = settings.height / 3;
			
			paginator.arrowRight.x = settings.width - paginator.arrowLeft.width + 20;
			paginator.arrowRight.y = settings.height / 3;
		}
...
internal class SaleItem extends LayerX {
		...
		public function SaleItem(item:Object, window:*, bonus:Boolean = false) {
			...
			var backType:String = 'itemBackingGold';
			//if (!bonus)
			//	backType = 'bonusBacking'
	
			if (item.count.decor == 1 || item.count.decor == 3) 
			{
			...
			var stripe:Bitmap = new Bitmap(Window.textures.saleRibbon);
			stripe.y = 3;
			stripe.x = 4;
			
			if (item.count.decor == 2 || item.count.decor == 3) 
			{				
				addChild(stripe);
			}
		...
		public function onPreviewComplete(data:Bitmap):void
		{
			...
			if (bitmap.width > background.width - 10) {
				bitmap.scaleX = bitmap.scaleY = (background.width - 10)/(bitmap.width);
			}
			if (bitmap.height > background.height - 10 ) {
				bitmap.height =  background.height - 10;
		...
		public function drawBttn():void
		{
			...
			priceBttn.settings['type'] = 'decorSale';
			priceBttn.settings['countText'] = count;
			addChild(priceBttn);
		...
		public function drawCount():void {
			...
			var priceLabel1:PriceLabelSales = new PriceLabelSales((item.price)?item.price:item.instance.cost[1]);
	...
	public function PriceLabelSales(price:Object) 
	{
		...
		switch(sID) {
			case Stock.COINS:
				...
				icon2 = new Bitmap(UserInterface.textures.goldenPearl, "auto", true);
				icon.scaleX = icon.scaleY = 0.74;
				icon2.scaleX = icon2.scaleY = 0.84;
				break;
			case Stock.FANT:
				icon = new Bitmap(UserInterface.textures.blueCristal, "auto", true);
				icon2 = new Bitmap(UserInterface.textures.blueCristal, "auto", true);
