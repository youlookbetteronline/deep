package wins 
{
	...
	public class DaylicsShopWindow extends Window
	{
		...
		public function DaylicsShopWindow(settings:Object = null):void
		{
			...
			settings["title"] = (User.inExpedition?App.data.storage[Stock.TENT].title:Locale.__e("flash:1442309571643"));
			settings["width"] = 720;
			settings["height"] = 540;
			...
			if (App.self.getLength(App.user.daylicShopData)==0) {
				for (var id:String in App.data.storage) {
					...
					if (/*item.hasOwnProperty('currency') &&*/ App.self.getLength(item.currency) && item.currency[Stock.SEA_STONE]) {
		...
		override public function drawBackground():void
		{
			...
			Load.loading(Config.getQuestIcon('preview', App.data.personages[6].preview), function(data:*):void {
				
				totem.bitmapData = data.bitmapData;
				
				if (App.data.personages[6].preview == 'squid') {
					totem.x = (settings.width - totem.width)/2;
					totem.y = -110;	
				}
			});
		...
		public function createContent():void {
			...
			//App.user.daylicShopData.sortOn('type', Array.NUMERIC);
		...
		override public function drawBody():void {
			...
			//бонуслист
			bonusList = new BonusList({3:App.user.stock.count(Stock.SEA_STONE)}, true, {
				hasTitle: false,
				extraWidth: true,
		...
		{
			if (!App.isSocial('DM','VK','MM','OK','FS','FB','NK')) 
			{
				return
			}
		...
		public function setContentSection(section:*,page:int = -1):Boolean {
			...
			if (sections.hasOwnProperty(section)) {
				...
				paginator.update();
				//settings.content.sortOn('type','order', Array.NUMERIC|Array.DESCENDING);
	...
	internal class DaylicsShopItem extends Sprite {
		...
		private function onLoadAnimate(swf:*):void {
			...
			if (spriteAnime.height > bg.height - 40 ) {
				...
				spriteAnime.scaleX = spriteAnime.scaleY;// = (background.height - 70) / (bitmap.height);
			}
			spriteAnime.x = bg.x + bg.width / 2;
			spriteAnime.y = bg.y + bg.height / 2 + 15/* + ((sprite.height > 100) ? ((sprite.height - 100) / 2) : 0)*/;
