package wins 
{
	...
	public class TechnoRecipeWindow extends RecipeWindow 
	{
		...
		protected function onThird(e:MouseEvent):void{
			/*if (settings.target.info.sID == 2177)
			{
				Locker.availableUpdate(false)
				return;
			}*/
			silentClose();
			settings.prodItem.recWin = new TechnoRecipeWindow( {
		...
		/*protected function onClimber(e:MouseEvent):void
		{
			close();
			settings.prodItem.recWin = new TechnoRecipeWindow( {
				title			:settings.title,
				fID				:settings.craftData[3].fid,
				busy			:false,
				hasDescription	:true,
				hasAnimations	:false,
				craftData		:settings.craftData,
				prodItem		:settings.prodItem,
				sID				:settings.craftData[3].sID,
				wID				:settings.wID,
				wSID			:settings.wSID,
				target			:settings.target
			});
