package wins 
{
	...
	public class AskWindow extends Window
	{
		...
		public function AskWindow(mode:int, settings:Object = null, callBack:Function = null)
		{
			...
			//settings['friendException'] = settings['exeption'] || [];
			...
			if (mode == MODE_NOTIFY_2) 
			{
				...
				//var dataFriends:Object = testObject; //не очень норм
				...
				for (var item:* in App.user.friends.keys)
				{
					...
					{
						//if (settings.exeption.indexOf(App.user.friends.keys[item].uid) != -1) 
							//continue;
		...
		override public function drawBody():void
		{
			...
			if (settings.content.length > 0){
				onCheckAllEvent(CheckboxButton.UNCHECKED);
				contentChange();
			}else{
		...
		protected function drawCheckAll():void 
		{
			...
			checkAllBox.x = settings.width - checkAllBox.width - 20;
			checkAllBox.y = settings.height - checkAllBox.height - 35;
		}
		private function onCheckAllClick(e:MouseEvent):void
		{
			//e.currentTarget.checked;
			
			onCheckAllEvent(e.currentTarget.checked);
			contentChange();
			//this.friend.checked = checkBox.checked;
			
		}
		
		private function onCheckAllEvent(_check:int):void
		{
			for (var i:int = 0; i < Numbers.countProps(settings.content); i++)
			{
				App.user.data.friends[settings.content[i].uid].checked = _check;
				settings.content[i].checked = _check;
			}
		}
		...
		protected function inviteCheckedEvent(e:MouseEvent):void 
		{
			if (settings.content.length <= 0)
				return;
