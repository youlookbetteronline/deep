	...
	public class ShopItem extends LayerX {
		...
		private function changeBacking():void 
		{
			...
			} else 
			{
				...
				if (item.type == 'Golden' || item.type == 'Walkgolden' || item.type == 'Energy' || item.type == 'Mfloors' || item.type == 'Tribute' || item.type == 'Booster')
		...
		private function onFindEvent(e:MouseEvent):void 
		{
			...
			else if (item && item.sid && item.sid == 2582)
			{
				...
				{
					if(item.market == 14/* && ShopMenu.getMapShop(App.user.worldID).index*/){
						new WorldsWindow( {
							title: Locale.__e('flash:1415791943192'),
							sID:	item.sid,
