package wins.elements
{
	...
	public class BankUsualItem extends LayerX
	{
		...
		public var settings:Object = {height: 73, width: 466, icons: false, sale: false, profit: 0, isBestsell: false, isActionGained: false};
		...
		public function BankUsualItem(item:*, window:*, _settings:Object = null)
		{
			...
			if (reward)
			{
				...
				bgMask.graphics.drawRect(0, 0, settings.bgMaskWidth + 10, background.height);
			...
			sprite.tip = function():Object
			{
				...
				if (item.type == "Plant")
				{
					return {title: item.title, text: Locale.__e("flash:1382952380075", [TimeConverter.timeToCuts(item.levelTime * item.levels), item.experience, App.data.storage[item.out].cost])};
				}
				else if (item.type == "Decor")
				{
					return {title: item.title, text: Locale.__e("flash:1382952380076", item.experience)};
				}
				else
				{
		...
		private function drawTopReward():void
		{
			...
			var rewText:TextField = Window.drawText(Locale.__e('flash:1493383312549'), {fontSize: 24, textAlign: "center", color: 0xff0000, borderColor: 0xffffff, textLeading: 8, multiline: true, width: 90});
			...
			{
				return {title: App.data.storage[rID].title, text: App.data.storage[rID].description}
			}
		...
		public function drawFlag():void
		{
			if (settings.isBestsell)
			{
				...
				var prcSetts:Object = {fontSize: 20, border: true, color: 0xfffeff, borderColor: 0x8c1639, textAlign: 'center'};
				...
				{
					Load.loading(Config.getImage('interface', preCurrView), function(data:Bitmap):void
					{
		...
		private function drawPremium():void
		{
			...
			if (settings.isActionGained)
			{
				...
				priceLabel.x = 255/* - 12 * int(App.isSocial('MX', 'YB'))*/;
				
				buyBttn.x = background.x + background.width - buyBttn.width - 15;
				/*var newStripe:Bitmap = new Bitmap(Window.textures.bankBonusDeepRibbon); //бонус в углу
				   newStripe.x = background.x + background.width - newStripe.width - 4;
				   newStripe.y = background.y + 4;
				   addChild(newStripe);*/
				
				/*profitLabel.visible = false;
				   superLabel.x = background.x + 160;
				   var coin:Bitmap = new Bitmap(UserInterface.textures.goldenPearl)
				   if (item.type == "Reals")
				   coin = new Bitmap(UserInterface.textures.blueCristal);
				   coin.smoothing = true;
				
				   //var coin:Bitmap = new Bitmap(UserInterface.textures.coinsIcon)
				
				   coin.x = superLabel.x + 45;
				   coin.y = superLabel.y + 10;
				   ...
				   plusLabel.x = coin.x - 3;
				   plusLabel.y = coin.y + 23;
				
				   prmsPlusFree.textAlign = 'left'
				   var freeLabel:TextField = Window.drawText(Locale.__e('flash:1382952380285') + '!', prmsPlusFree)
				   freeLabel.x = plusLabel.x + plusLabel.textWidth + 3;
				   freeLabel.y = plusLabel.y;*/
				
				var bcount:uint = bonusVal;
				
				//var size:uint = 100; 
				//var redLine:Shape = new Shape(); 
				
				// red circular shape 
				//redLine.graphics.beginFill(0xFF0000); 
				//redLine.graphics.moveTo(0, counterLabel.textHeight); 
				//redLine.graphics.curveTo(counterLabel.textWidth - 50, counterLabel.textHeight - 50, counterLabel.textWidth, 0);
				...
				var prmsCountLbl:Object = {color: (item.type != 'Reals') ? 0xf9c83b : 0x9dfcff, borderColor: (item.type != 'Reals') ? 0x7b4004 : 0x335c7b, width: 120, border: true, multilnine: true, wrap: true, textAlign: "left", autoSize: "center", fontSize: 40}
		...
		public function drawOldValue():void
		{
			var prmsCountLbl:Object = {color: 0x3f7600,//(item.type!='Reals')?0xf9c83b:0x9dfcff,  
				//borderColor:(item.type!='Reals')?0x7b4004:0x335c7b,
				width: 120, border: false, multilnine: true, wrap: true, textAlign: "left", autoSize: "center", fontSize: 30}
			
			var oldValue:Number = settings.profit;
			if (oldValue == 0) return;
			var _result:int = Math.round(price * settings.profitValue);
			var oldValueLable:TextField = Window.drawText(String(_result), prmsCountLbl);
			var redLine:Bitmap = new Bitmap(Window.textures.redLines);
			redLine.smoothing = true;
			
			oldValueLable.x = priceLabel.x + 20;
			priceLabel.y += 18;
			oldValueLable.y = priceLabel.y - 30;
			
			redLine.x = oldValueLable.x - 5;
			redLine.y = oldValueLable.y + oldValueLable.textHeight - redLine.height - 4;
			
			addChild(oldValueLable);
			addChild(redLine);
		...
		private function checkLabels():void
		{
			...
			////profitLabel.border = true;
			//profitLabel.height = 56;
		...
		{
			var text:String;
			price = item.socialprice[App.social];
			//var price:Number = item.socialprice[App.social];
			...
			if (App.isSocial('FB'))
				priceVal = Locale.__e(text, [int(price * 100) / 100])
			var bttnSettings:Object = {caption: Locale.__e('flash:1382952379751'), fontSize: 24, width: 100, height: 40, hasDotes: false};
			...
			var pvalPars:Object = {color: 0x793900, border: false, textAlign: "left", autoSize: "center", fontSize: 28};
			
			priceLabel = Window.drawText(priceVal, pvalPars)
			
			addChild(priceLabel);
			//	priceLabel.x = 305;
			priceLabel.x = 300 - 50 /*- 12 * int(App.isSocial('MX', 'YB'))*/;
			priceLabel.y = background.y + (background.height - priceLabel.height) / 2;
			
			if (!checkDmaterials || (checkDmaterials && App.user.stock.count(Numbers.firstProp(item.dmaterials).key) < 1))
				drawOldValue()
		...
		{
			title = Window.drawText(String(item.title), {color: (item.type != 'Reals') ? 0xffffff : 0xcde8ff, borderColor: (item.type != 'Reals') ? 0x7b4004 : 0x242578, textAlign: "center", autoSize: "center", fontSize: 24, textLeading: -6, multiline: true, wrap: true, width: background.width - 40});
			title.y = 5;
		...
		private function drawCount():void
		{
			...
			if (Payments.isExtraActive(item.sid))
			{
			...
			counterLabel = Window.drawText(sighnTxt + String(item.price[ind])/*(item.price[ind] + ((App.data.money[App.social].enabled && App.data.money[App.social].date_to > App.time)?buyObject.extra:0))*/, {fontSize: (settings.isActionGained == true) ? 30 : 36, color: (item.type == 'Reals') ? 0x9dfcff : 0xffdf34, borderColor: (item.type != 'Reals') ? 0x7b4004 : 0x335c7b, autoSize: "left"});
		...
		private function buyEvent(e:MouseEvent):void
		{
			...
			if (App.social == 'FB')
			{
				object = {id: buyObject.id, type: buyObject.type, callback: function():void
				{
					bonusAnim();
				}};
			...
			else
			{
				...
				object = {money: buyObject.type, type: 'item', item: buyObject.type + "_" + buyObject.id, votes: buyObject.votes, sid: buyObject.id, count: buyObject.count + ((App.data.money.enabled && App.data.money.date_to > App.time) ? buyObject.extra : 0), title: buyObject.title, description: buyObject.description, icon: buyObject.icon, tnidx: App.user.id + App.time + '-' + buyObject.type + "_" + buyObject.id, callback: function():void
				{
					bonusAnim();
				}}
			}
