		...
		override public function search(query:String = "", isCallBack:Boolean = true):Array {
			...
			function useFilters(_uid:String):Boolean
			{
				...
				if (wlFilter) {
					...
					for (var wItemSid:* in App.user.friends.data[_uid].wl) 
						if (wItemSid == settings.win.icon.ID) {
							if (App.user.friends.data[_uid].wl[wItemSid].hasOwnProperty('f')) {
								if (App.user.friends.data[_uid].wl[wItemSid]['f'].indexOf(App.user.id) != -1 || App.user.friends.data[_uid].wl[wItemSid]['f'].length == 0) {
									check = true;
								}
							}
						}
