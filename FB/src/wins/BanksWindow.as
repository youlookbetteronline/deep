package wins 
{
	...
	//	private var clickCont:LayerX;
		...
		//	drawCards();
			...
			{
				lable1 = new Bitmap();
				Load.loading(Config.getImageIcon('money', 'secure_logos'), function(data:Bitmap):void {
					lable1.bitmapData = data.bitmapData;
					lable1.x = 65;
					lable1.y = settings.height - 120 + 25;
				});
				
				lable2 = new Bitmap();
				Load.loading(Config.getImageIcon('money', 'secure_lock'), function(data:Bitmap):void {
					lable2.bitmapData = data.bitmapData;
					lable2.x = settings.width - 65 - lable2.width;
					lable2.y = settings.height - 120 + 25;
				});
		...
		private function checkAction():void 
		{
			if ((App.data.money && App.time >= App.data.money.date_from && App.time < App.data.money.date_to && App.data.money.enabled == 1) || App.user.money > App.time) {
				...
				var stripe:Bitmap = new Bitmap(Window.textures.bankStripe);
				stripe.smoothing = true;
				stripe.rotation = 25;
				
				actionCont.addChild(stripe);
				...
				actionTitle.y = 35;
				actionTitle.width = actionTitle.textWidth + 5;
				...
				actionTime.y = actionTitle.y + actionTitle.textHeight - 10;
				actionTime.x = 0
				actionTime.width = actionTime.textWidth + 5;
				...
				bodyContainer.addChildAt(actionCont,0);
		...
		public function DrawMoney():void 
		{
			...
			if (App.isSocial('FB')) {
				if (history.section == COINS)
					profitList = [0, 1.6032064, 2.002002, 2.0010005, 2.0004, 2.0002];
					//profitList = [0, 8, 20, 40, 100, 200];
					//profitList = [30, 50, 80, 100, 200];
				else 
					profitList = [0, 0, 1.2012, 1.5007, 2.0004, 2.0002];//, 500];
					//profitList = [0, 0, 12, 30, 100, 200];//, 500];
					//profitList = [30, 50, 80, 100, 250];
			}
			profitList.reverse();
			items = [];
			var X:int = 35;
			...
			for (var i:uint = paginator.startCount; i < paginator.finishCount; i++) 
			{
				...
				if (i == 0 || i == 2)
				{
					params['bgMaskWidth'] = 546;
				}else{
					params['bgMaskWidth'] = 466;
				}
