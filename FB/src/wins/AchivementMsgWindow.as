package wins 
{
	...
	public class AchivementMsgWindow extends Window
	{
		...
		override public function drawBody():void 
		{
			...
			this.x = -int(App.self.stage.stageWidth/2) + this.width/2 + 138;
			this.y = int(App.self.stage.stageHeight/2) - this.height + 40;
		...
		private function drawStars():void 
		{
			...
			var starGlow:Bitmap;
			for (i = 0; i < numMis; i++ ) {
				var star2:Bitmap = new Bitmap(UserInterface.textures.expIco);
	...
	private function glowStar(star:Bitmap):void
	{
		...
		private var margY:int = 1;
		private var tweenGlow1:TweenLite;
		private var tweenGlow2:TweenLite;
		private function glowStar(star:Bitmap):void
		{
			tweenGlow1 = TweenLite.to(star, 0.8, {x:star.x-margX, y:star.y-margY, alpha:1, scaleX:-0.9, scaleY:0.9, ease:Elastic.easeOut, onComplete:function():void {
				tweenGlow2 = TweenLite.to(star, 0.8, {x:star.x+margX, y:star.y+margY, alpha:0, scaleX:-0.7, scaleY:0.7, /*ease:Elastic.easeIn,*/ onComplete:function():void {
					glowStar(star);
				}});
			}});
		}
