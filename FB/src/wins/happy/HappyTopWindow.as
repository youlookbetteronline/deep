package wins.happy {
	...
	public class HappyTopWindow extends Window {
		...
		public function HappyTopWindow(settings:Object = null,sh:Boolean=true) {
			info = App.data.storage[settings.sid];
			info.sid = settings.sid;
			_bttOk = sh;
		...
		private function creatTop():void 
		{
			for (var top:* in App.data.top)
				...
				if (App.data.top[top].target == info.sid) 
				{
					_top = top;
					break;
				}
		...
		override public function drawBody():void {
			...
			var bttn:Button = new Button( {  
				width:194, 
				height:53, 
				caption:Locale.__e('flash:1432128036600') 
				} );
			bttn.x = (settings.width - bttn.width) / 2;
			bttn.y = settings.height - bttn.height - 65;
			...
			bodyContainer.addChild(descriptionLabel);	// FB && NK
			...
			//descriptionLabel2.x = (settings.width-descriptionLabel2.width)/2-120; // FB && NK
			descriptionLabel2.x = 80;// (settings.width - descriptionLabel2.width) / 2;
			//descriptionLabel2.y = bodyContainer.y + 310; // FB && NK
			descriptionLabel2.y = 65;//bodyContainer.y + 130;
		...
		if(_top>0)
			{	
				...
				var topBttnText:String = Locale.__e('flash:1473845055751',[App.data.top[_top].limit]);
					top100Bttn = new IconButton(Window.textures.octupusHand,{
							width:		160,
							//height:		90,
							caption:	topBttnText
						});
					...
					//top100Bttn.x = background.x +background.width-top100Bttn.width- 125;		//FB
					top100Bttn.x = background.x +background.width - top100Bttn.width - 75;		//FB
					//if()
					top100Bttn.y = background.y +297;			//FB
					//top100Bttn.y = background.y +57;
		...
		private function drawpoints():void 
		{
			...
			var backgST:Bitmap = Window.backing(255, 160, 40, "shopSpecialBacking");
...
////////
		...
		protected function drawTimer():void {
			var timerBacking:Bitmap = new Bitmap(Window.textures.questIconBacking/*mapGlow*/, 'auto', true);// Window.backingShort(150, 'seedCounterBacking'); 		 //D
			timerBacking.x =-10;
			timerBacking.y = -40;
...
internal class InfoWindowItem extends LayerX {
	...
	public function InfoWindowItem(image:String,i:int, _win:Window,top:Boolean=false) {
		...
		if (top)
			//txt = "Топ " + i;					//D ????
			txt = Locale.__e("flash:1473845055751",[i]);					//D ????
		else
			//txt = "Этап " + i;
		...
		var descriptionLabel:TextField = Window.drawText(App.data.storage[sid].title,descPrms);
			...
			descriptionLabel.y = titlLabel.y + titlLabel.height+5;
