package wins 
{
	...
	public class StockWindow extends Window
	{
		...
		public function StockWindow(settings:Object = null):void
		{
			...
			settings["title"] = /*(User.inExpedition?App.data.storage[Stock.TENT].title:*/Locale.__e("flash:1382952379767");// );
			settings["width"] = 690;
			settings["height"] = 560;
...
//	private var countMinistock;
	...
	public function StockItem(item:*, window:*):void {
		...
		if ((!World.canBuyOnThisMap(item.sid) || App.user.worldID == User.NEPTUNE_MAP) && item.type != 'Energy'&& item.type != 'Material')
			drawFindBttn();
