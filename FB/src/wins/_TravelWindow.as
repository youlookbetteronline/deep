package wins
{
	...
	{	
		...
		private var lines:Vector.<Shape> = new Vector.<Shape>();
		...
		public function generateRadarLocation():void
		{
			...
			for each(var _item:* in App.user.openedRadarMaps)
			{
				...
				//radarItem.x = radarPosition.x * scale;
				//radarItem.y = radarPosition.y * scale;
		...
		private function clearRadarItems():void
		{
			...
			//for (var i:int = 0; i < radarsVector.length; i++)
			//{
				//radarsVector[i].dispose();
				//radarsVector.splice(radarsVector.indexOf(radarsVector[_item]), 1);
			//}
		...
		public static function getPages():Array 
		{
			...
			{
				/*if (User.inUpdate(App.user.maps[i].sID)) ременно
				{*/
		...
		public static var images:Object = {
			...
			1: { bgColor:0x310f27, title:App.data.storage[938].title, bmd:null},
			2: { bgColor:0x310f27, title:App.data.storage[1766].title, bmd:null},
			3: { bgColor:0x310f27, title:App.data.storage[3201].title, bmd:null}*/
		};
		...
		override public function contentChange():void 
		{
			...
			for (var s:* in pages[paginator.page]) 
			{
				...
				if(Stock.isExist(1242) == false && s == User.SHARK_LOCATION)
					item.visible = false;
		...
		private function drawPageBttns():void
		{
			...
			for (var s:* in App.user.maps)
			{
				var bttn:Button = new Button( {
					...
					fontSize:	18,
					multiline:  true,
					bgColor:	[0xfdd349, 0xf5a920],
					borderColor:[0xffeede, 0xbcbaa6],
					fontBorderColor: 0x426e7b,
					bevelColor:	[0xfff17e, 0xc57d0e]
				});
...
internal class RadarItem extends LayerX {
	...
	public function onClick(e:MouseEvent = null):void 
	{
		...
		//{
		if (App.user.worlds.hasOwnProperty(settings['sID']))
		{
			if(App.user.worldID != settings['sID'])
