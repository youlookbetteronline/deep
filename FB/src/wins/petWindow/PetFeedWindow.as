package wins.petWindow
{
	...
	public class PetFeedWindow extends Window 
	{
		...
		private var needSprite:LayerX;
		private function drawNeedText(value:int = 0):void 
		{
			if (!value)
				value = _pet['bountiesLeft'];
				
			if (needSprite && needSprite.parent)
				needSprite.parent.removeChild(needSprite);
				
			needSprite = new LayerX();
			...
			needSprite.addChild(_textBonusesLeft);
			
			_textBonusesLeftValue = Window.drawText(String(value), 
			{
			...
			_textBonusesLeftValue.x = _textBonusesLeft.x + _textBonusesLeft.textWidth + 8;
			needSprite.addChild(_textBonusesLeftValue);
			...
			_textPickActions.x  = _textBonusesLeftValue.x + _textBonusesLeftValue.textWidth + 10;
			_textPickActions.y  = _textBonusesLeftValue.y + (_textBonusesLeftValue.height - _textPickActions.height) / 2;
			needSprite.addChild(_textPickActions);
			
			bodyContainer.addChild(needSprite);
			needSprite.x = (settings.width - needSprite.width) / 2;
			needSprite.y = settings.height - 72 -needSprite.height;
		}
