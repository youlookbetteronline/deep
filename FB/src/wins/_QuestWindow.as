package wins 
{
	...
	public class QuestWindow extends Window
	{
		...
		override public function drawBody():void 
		{
			...
			Load.loading(Config.getQuestIcon('preview', characterPerv), function(data:*):void 
			{
				...
				switch(App.data.personages[quest.character].preview) 
				{
					...
					case 'professor':
						character.x = -(character.width / 4) * 3 - 20;
						character.y = -110 + addY;
					break;
					case 'chief':
						character.x = -(character.width / 4) * 3 - 20;
						character.y = -130 + addY;
					break;
					case 'Zikimus':
						character.x = -(character.width / 4) * 3 - 20;
						character.y = -130 + addY;
					...
					case 'zikimund':
						character.x = -(character.width / 4) * 3 - 20;
						character.y = -200 + addY + 50;
					break;
					case 'ingrid':
						character.x = -(character.width / 4) * 3 - 20;
						character.y = -200 + addY + 110;
					break;
					case 'prince_turn':
						character.x = -(character.width / 4) * 3 - 20;
						character.y = -200 + addY + 110;
			...
			/*if(_winContainer){
				bonusBackgroutnd.x = bg.x + _winContainer.x + (_winContainer.width - bonusBackgroutnd.width) / 2;
			} else {
				bonusBackgroutnd.x = bg.x  + ( bonusBackgroutnd.width) / 2;	
			}*/
			...
			titleTextCenter.x = (settings.width / 2 - titleTextCenter.width / 2) /*+ 20*/;
			...
			bonusList.y = bonusBackgroutnd.y + 10/*bonusList.height / 2*/;
			...
			okBttn.y = background.height - okBttn.height + 7;
			...
			/*
			separator = Window.backingShort(settings.width - 120, 'divider');
			separator.alpha = 0.8;
			bodyContainer.addChildAt(separator, 0);
			separator.x = 60;
			separator.y = okBttn.y + okBttn.height / 2 - separator.height / 2 - 5;*/
			
			if (isDuration) {				
				drawTime();
			}
			
			/*crab = new Bitmap(Window.textures.crabQuest);
			crab.x = background.width/2 - 50;
			crab.y = - 55;
			bodyContainer.addChild(crab);*/
			
			exit.scaleX = exit.scaleY = 1.1;
			//exit.smoothing = true;
			exit.x = background.width - 65;
			exit.y = background.y;
		}
		...
		override public function contentChange():void {
			...
			background = backing(settings.width, bgHeight , 50, 'workerHouseBacking');
			
			var background2:Bitmap  = backing2(settings.width - 60, bgHeight - 54, 40, 'capsuleWindowBackingPaperUp', 'capsuleWindowBackingPaperDown');
			/*if (missions.length == 3)
			{
				bodyContainer.y = 50;
				exit.y = -45;
...
internal class Mission extends Sprite {
	...
	public function Mission(qID:int, mID:int, window:*)
	{
		...
		if(mission.func == 'sum'){
			...
			//text = have + '/' + mission.need;
			}else if ((mission.event == "grow") && (qID == 548 || qID == 556 || qID == 748)) {
