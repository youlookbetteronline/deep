	...
	public class Window extends Sprite
	{
		...
		public function drawBubbles():void 
		{
			...
			for (var i:int = 0; i < Window.isOpen.settings.bubblesCount; i++) 
			{
				...
				bubblesCoords[i] = {x:bubble.x, y:bubble.y, speed:1 / (bubble.width / 10) + Math.random() * .2};
			}
			...
			if (stage){	
				...
				{	
					...
					if (bubblesVector[i].y + bubblesVector[i].height < heightDifference)
					{
						...
						//bubblesCoords[i].x = 20 - Math.random() * 150;
						//bubble1.y = stage.stageHeight;
						bubbleParallax();
					}
				}
			}
		...
		private function bubbleParallax():void
		{
			...
			/*trace("Mouse x " + mouseX + "/ "+stage.stageWidth);
			trace("Mouse y " + mouseY + "/ " + stage.stageHeight);*/
			...
			for (var i:int = 0; i < Window.isOpen.settings.bubblesCount; i++) 
			{
				...
				/*if (i < Window.isOpen.settings.bubblesCount / 2){
					bubblesVector[i].scaleX = bubblesVector[i].scaleY = 0.5 + 0.3 * mouseX / -stage.stageWidth + 0.2 * mouseY / stage.stageHeight;
				}else{
					bubblesVector[i].scaleX = bubblesVector[i].scaleY = 0.5 + 0.3 * mouseX / stage.stageWidth + 0.2 * mouseY / stage.stageHeight;
				}*/
				//bubblesVector[i].scaleX = bubblesVector[i].scaleY = 0.7 + 0.3 * Math.abs(mouseX - bubblesVector[i].x);
				bubblesVector[i].x += (1 - bubblesVector[i].scaleX) * bubblesVector[i].width;
				//bubblesVector[i].y += (1 - bubblesVector[i].scaleY) * bubblesVector[i].height;
			}
		}
		...
		protected function drawRibbon():void 
		{
			...
			if (ribWidth < 320)
				ribWidth = 320;
			titleBackingBmap = backingShort(ribWidth, 'ribbonGrenn', true, 1.3);
