...
/*
		...
		private function onStartClick(event:GGameEvent):void {
			...
			if(App.user.stock.checkAll(_model.startPrice/*, true*/) && App.user.stock.count(_model.catchPriceSID) > 0)
			{
				_model.gameStartCallback();
				_startPanel.enabled(false);
			}
			else if (!App.user.stock.checkAll(_model.startPrice))
			{
			...
			{
				if (App.user.stock.count(_model.catchPriceSID) == 0)
				{
					var _content:Array = PurchaseWindow.createContent("Energy", {inguest:0, view:App.data.storage[_model.catchPriceSID].view},_model.catchPriceSID)
					new PurchaseWindow( {
						popup:		true,
						width:		600,
						itemsOnPage:_content.length,
						content:	_content,
						title:		App.data.storage[_model.catchPriceSID].title,
						description:Locale.__e('flash:1472652747853')
					}).show();
					return;
				}
