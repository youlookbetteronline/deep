package wins 
{
	...
	public class InstancePassingWindow extends BuildingWindow 
	{
		...
		private function drawMainBack():void 
		{
			
			var underBg:Bitmap = Window.backing2(230, 316, 45, 'snstSmallBackingTop', 'insttSmallBackingBot');
			underBg.alpha = 1;
			underBg.x = 231;
			underBg.y = 200;
			//bodyContainer.addChildAt(underBg, 0);
		}
		
		
		override public function drawBackground():void
		{
			backgound = new Bitmap();
				Load.loading(Config.getImage('instance', 'InstanceBacking'), function(data:Bitmap):void
			{
			backgound.bitmapData = data.bitmapData;
		...
		private function showPersonages():void {
			...
			for (var i:int = 0; i < persSidArr.length; i++) 
			{
				...
				case "boy":
					icon =  new Bitmap(Window.textures.charBoyIco);
					icon.x = stand.x + (stand.width - icon.width) / 2 +2;
					icon.y = stand.y + (stand.height - icon.height) / 2 - 15;
				break;
				case "girl":
					icon =  new Bitmap(Window.textures.charGirlIco);
					icon.x = stand.x + (stand.width - icon.width) / 2 +2;
					icon.y = stand.y + (stand.height - icon.height) / 2 - 13;
				break;
				case "undeadWoman":
					icon =  new Bitmap(Window.textures.charGirlIco);
					icon.x = stand.x + (stand.width - icon.width) / 2 + 2;
			...
			stand = new Bitmap(Window.textures.instCharBacking);
				...
				countSlaves = Window.drawText("x "+String(settings.roomInfo.require[Techno.TECHNO]), {
				fontSize:40,
				color:0xffffff,
				autoSize:"left",
				borderColor:0x734418
			});
			container.addChild(countSlaves);
			countSlaves.x = stand.width-countSlaves.width+10;
			countSlaves.y = stand.height-countSlaves.height+10;
		}
		...
		private function onImproveTime(e:MouseEvent):void 
		{
			...
			settings.target.clickable = false;
		...
		private function onImproveTimeComplete(error:int, data:Object, params:Object):void
		{
			...
			settings.target.hasProduct = true;
							close();
		...
		public override function close(e:MouseEvent = null):void {
			settings.target.clickable = true;
			super.close();
		}
		...
		private function drawOuts():void 
		{
			...
			var underBg:Bitmap = Window.backing2(200, 286, 45, 'snstSmallBackingTop', 'insttSmallBackingBot');
			underBg.alpha = 0.5;
			//underBg.alpha = 1;
			...
			outsTitle.x = (underBg.width - outsTitle.textWidth) / 2 -10;
		...
		private var countSlaves:TextField;
		...
		public function drawFriendsInfo():void
		{
			...
			title.x = (underBg.width - title.textWidth) / 2 - 16;
			...
			descFriends.x = (underBg.width - descFriends.textWidth) / 2 -20;
			...
			inviteBttn.x = (underBg.width - inviteBttn.width) / 2 -20;
...
internal class OutItem extends Sprite {
	...
	public function updateProgress(value:int = 0):void
	{
		...
		if (settings.window.arrHeroesSids.length == 0 && settings.target.roomInfo.sID != 202) 
		{
