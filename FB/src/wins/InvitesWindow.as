package wins 
{
	...
	public class InvitesWindow extends Window 
	{
		...
		public function InvitesWindow(settings:Object=null) 
		{
			...
			settings["subwindow"] = InvitesWindow.REQUESTS;
		...
		override public function drawBody():void 
		{
			...
			drawMirrowObjs('storageWoodenDec', 5, settings.width - 4, settings.height - 120);
			drawMirrowObjs('storageWoodenDec', 5, settings.width - 5, 40, false, false, false, 1, -1);
			
			var decor:Bitmap = Window.backingShort(settings.width - 120, 'divider');
			decor.alpha = 0.8;
			...
			checkBox = new CheckboxButton( {
				captionChecked:Locale.__e('flash:1415873334680'),
				captionUnchecked:Locale.__e('flash:1415873334680'),
				textFieldWidth:280,
				checked:int(2 - App.user.hidden)
			});
			checkBox.x = 45;
			checkBox.y = settings.height - 100;
		...
		private var bttns:Object = {
			0:{label:'flash:1382952380191', name:0, description:'flash:1415720671965'},
			1:{label:'flash:1382952380190', name:1, description:'flash:1415720819321'},
			2:{label:'flash:1382952380073', name:2, description:'flash:1415720935360'}
		};
		...
		private function drawDescription():void {
			...
			descText.width = settings.width - 40;
		...
		private function drawSearch():void {
			back = backing(settings.width - 80, 200, 50, 'buildingDarkBacking');
			...
			shareInfo.x = searchFriend.x + 410;
			shareInfo.y = searchFriend.y - 15;
			swContainer.addChild(shareInfo);
			Log.alert('App.invites.randomProfiles');
				//Log.alert(App.invites.randomProfiles);
				Log.alert(App.self.getLength(App.invites.randomProfiles));
			if (App.self.getLength(App.invites.randomProfiles) > 0) {
				if (App.isSocial('MX','YB','AI')) 
				{
				chooseRandom = [];
				for each(var friend:Object in App.invites.randomProfiles) {
				if (friend.hasOwnProperty('_id')&&App.invites.invited[friend._id] == undefined)
				
					chooseRandom.push(friend);
					if (chooseRandom.length==4) 
					{
						break
					}	
				}}else 
				...
				if (chooseRandom.length == 0) {
					...
					while (true) {
						...
						for (var j:int = 0; j < chooseRandom.length; j++) {
							if (chooseRandom[j].hasOwnProperty('_id')) 
							{
						...
						if (chooseRandom.length >= 3 || chooseRandom.length == App.invites.random.length) {
							break;
						}
					}
		...
		private function drawRequests():void {
			back = backing(settings.width - 60, 380, 50, 'buildingDarkBacking');
			back.x = (settings.width - back.width) / 2;
			back.y = 110;
			swContainer.addChild(back);
		...
		private const ITEMS_WIDTH:int = 150;
		...
		public function initContent():void {
			...
			for (var fID:String in App.invites.invited) {
				...
				if(friend){
					...
					friend['type'] = FriendItem.CANCEL;
					content.push(friend);
				}
			}
			
			for (var fID2:String in App.invites.requested) {
				
				var friend2:Object = App.invites.requestedProfiles[fID2];
				if(friend2){
					friend2['time'] = App.invites.requested[fID2];
					friend2['type'] = FriendItem.APPLY;
					content.push(friend2);
				}
			}	
			}else 
			{
			...
			for each (object in App.invites.requested) {
				var friend3:Object = App.invites.requestedProfiles[object._id];
				if(friend3){
				if (!object['photo']) object['photo'] = '';
...
internal class FriendItem extends LayerX {
	...
	private function draw():void {
		...
		deleteBttn = new ImageButton(Window.textures.closeBttnSmall);
		...
		if (link == 'none') {
			onLoad(new Bitmap(Window.textures.frSlot, 'auto', true));
		}else {
		...
		if (params.type == ADD) {
			...
			if (params.info.hasOwnProperty('_id')&&App.invites.invited.hasOwnProperty(params.info._id)) {
				addedInfo();
			}else{
				addBttn = new Button( {
					width:		back.width - 36,
		...
		if (params.type == CANCEL) {
			cancelBttn = new Button( {
				...
				fontSize:		(App.isSocial('YB','MX','AI'))?14:22,
	...
	private function get link():String {
		if (params['info'] && params['info'].hasOwnProperty('photo')&& params['info']['photo']) {
...
internal class SearchFriend extends Sprite {
	...
	public function SearchFriend(window:*) {
		...
		descIDLabel = Window.drawText(Locale.__e('flash:1415783202887'), {
			width:			420,
			textAlign:		'center',
			color:			0xf9ffff,
		...
		iconBack = new Bitmap(Window.textures.persIcon, 'auto', true);
		iconBack.x = 40;
		...
		akaLabel.y = iconBack.y - 4;
		addChild(akaLabel);
		
		searchPlaha = new Bitmap(UserInterface.textures.searchBttn, 'auto', true); 
		...
		searchInputLabel.maxChars = 100;
		searchInputLabel.addEventListener(TextEvent.TEXT_INPUT, onTextInputEvent);
		
		addChild(searchInputLabel);
		
		clearBttn = new ImageButton(UserInterface.textures.stopIcon);
		...
		addBttn.y = iconBack.y + iconBack.height;
	...
	private function onTextInputEvent(e:TextEvent):void {
		setTimeout(updateTextState, 100);
	}
	private function updateTextState():void {
		if (searchInputLabel.wordWrap == false && searchInputLabel.textWidth > searchInputLabel.width) {
			searchInputLabel.height = Math.ceil(searchInputLabel.textWidth / searchInputLabel.width) * 26.5;
			searchInputLabel.wordWrap = true;
			searchInputLabel.multiline = true;
		}else if(searchInputLabel.numLines == 1) {
			searchInputLabel.wordWrap = false;
			searchInputLabel.multiline = false;
			searchInputLabel.height = 24.5;
		}
		
		searchPlaha.y = searchInputLabel.y + searchInputLabel.height * 0.5 - searchPlaha.height * 0.5;
		clearBttn.y = searchInputLabel.y + searchInputLabel.height * 0.5 - clearBttn.height * 0.5;
		searchBttn.y = searchInputLabel.y + searchInputLabel.height + 12;
		
		searchBack.graphics.clear();
		searchBack.graphics.beginFill(0xf1d9ab, 1);
		searchBack.graphics.drawRoundRect(0, 0, 170, searchInputLabel.height, 10, 10);
		searchBack.graphics.endFill();
	}
	...
	private function searchComplete(data:Object):void {
		...
		if (data && data._id) {
			...
			drawAva(data);
			akaLabel.text = data['aka'] || data['first_name']||defaulText;
		}else {
			addBttn.state = Button.DISABLED;
			drawAva(null);
			findID = '';
...
internal class ShareInfo extends Sprite {
	...
	public function ShareInfo() {
		...
		var back:Bitmap = Window.backing2(260, 190, 50, 'questTaskBackingTop', 'questTaskBackingBot');
		back.x = 0;
		addChild(back);
		...
		descTellLabel.text = Locale.__e('flash:1415783444367');
