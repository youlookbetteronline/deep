package api 
{
	...
	public class VKApi 
	{
		...
		private function takeFriends():void
		{
			...
			counter ++;
			VK.api('execute',{
				code:'return {"profile" : API.getProfiles({"uids":['+uids.join(',')+'],"https":1,"fields":"uid,first_name,last_name,photo,sex"})};', test_mode:Config.testMode
		...
		{
			if (data.appFriends.length > 0){ // не забыть убрать!
				//Log.alert('paths:');
				//paths = chunk(data.appFriends, 200);
				//Log.alert(paths);
				//takeFriends();
				getOtherFriends();
			}else {
