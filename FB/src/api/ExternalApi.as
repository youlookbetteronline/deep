package api 
{
	...
	public class ExternalApi 
	{
		...
		public static function checkTargetGroupMember(groupsIds:Array, callback:Function):void
		{
			if (!ExternalInterface.available)
				return	
			...
			switch(App.self.flashVars.social)
			{
				...
				case "FB":
						...
						ExternalInterface.call("isTargetGroupMember", groupsIds);
					break;
				case "OK":
		...
		public static function apiWallPostEvent(type:uint, bitmap:Bitmap, owner_id:String, message:String, sID:uint = 0, callback:Function = null, settings:Object = null):void {
			...
			if (App.self.stage.displayState != StageDisplayState.NORMAL&& (App.self.flashVars.social!='FB')) {
			...
			switch(App.self.flashVars.social) {
				case 'VK':
						...
						{	
							...
							ldr.addFile(pngStream, "image.png", "file", 'image/png');
							ldr.load('http://tribe.islandsville.com/iframe/upload.php');
