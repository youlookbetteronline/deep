	...
	public class Map extends LayerX
	{
		...
		public function touches(e:MouseEvent):void {
			...
			if (length > 0) {
				for each(var touch:* in touched) {
					bmp = touch.bmp;
					if (!touch.bitmap.bitmapData && touch.hasOwnProperty('animeTouch') && touch.animeTouch.bitmapData.getPixel(touch.animeTouch.mouseX, touch.animeTouch.mouseY) == 0)
					{
						touch.touch = false;
				...
				for (var i:int = length-1; i >= 0; i--) {
					...
					else if (under[i].parent && under[i].parent.parent is Unit)
					{
		...
		public function touch():void
		{
			...
			if (touched.length && (touched.length > 0) && !(touched[0] is Unit || touched[0] is Monster)) return;
			if (touched.length > 0)
			{
				var unit:* = touched[0];
			}else 
