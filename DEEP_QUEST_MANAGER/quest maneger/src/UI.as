package  
{
	...
	public class UI 
	{
		...
		public static function backing2(width:int, height:int, padding:int = 50, texture:String = "windowBacking", texture2:String = "windowBacking", koefBetweenTextures:int = 0):Bitmap {
			...
			public static function getColorUpdate(upd:String):int
			{
				if (!upd)
					return 0;
				var countUpdate:int = Numbers.countProps(App.data.updates)
				var orderUpdate:int = App.data.updates[upd].order - 1;
				var colorUpdate:int = 16776960 / countUpdate * orderUpdate;
				return colorUpdate;
			}
	}
