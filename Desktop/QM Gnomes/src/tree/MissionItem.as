package tree 
{
	...
	public class MissionItem extends Sprite {
		...
		public function MissionItem(qID:int, mID:int) {
			...
			if (Numbers.countProps(User.quests) > 0 && User.quests.hasOwnProperty(qID))
			{
				if(User.quests[qID].hasOwnProperty(mID))
					text = User.quests[qID][mID] + '/' + mission.need;
				else
					text = 0 + '/' + mission.need;
			}else{
				//if(mission.func == 'sum'){
