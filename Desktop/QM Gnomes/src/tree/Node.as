package tree 
{
	...
	public class Node extends LayerX
	{
		...
		public function get _children():Array{
			return _mchildren;
		}
		private function init (rootID:uint):void
		{
			...
			for each ( var quest:Object in _quests)
			{
				...
				{
					if (_quests[_id].parent != 0 && quest.parent[0] == _id)
			...
			if (_id == 71)
				trace();
			if (_quests[_id].hasOwnProperty('parent') && !(_quests[_id].parent is Array) && !(_quests[_id].parent is int))
				trace();
			if (_quests[_id].hasOwnProperty('parent') && _quests[_id].parent != 0 && _quests[_id].parent.length > 0){
				
				parents = _quests[_id].parent;
				var _finded:Boolean;
				for each(var _pID:* in parents){
					if (_quests[_pID].update == _quests[_id].update){
						_finded = true;
						break;
					}
				}
				if(_finded)
					...
					mparent = _quests[_id].parent[0];
					
				if (parents.indexOf(68) != -1)
					trace();
			}
			else
				mparent = _id;
		...
		public function draw():void
		{
			...
			var _color:uint;
			if(quest.update != "")
				_color = App.data.updates[quest.update].color;
			else
				_color = 0;
				
			backing.filters = [new GlowFilter(_color, 1, 15, 15, 2, 1, true)];
