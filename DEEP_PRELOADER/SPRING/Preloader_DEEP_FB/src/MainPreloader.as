package {
	...
	[SWF ( width="1600", height="700", backgroundColor="#042238", allowsFullScreen = true) ]
	...
	public class  MainPreloader extends Sprite
	{
		...
		[Embed(source="MainLogoSpring.png")]
		...
		[Embed(source="start_screen_spring.jpg")]
		...
		{	
			...
			mainLogoDeep.y = stage.stageHeight - mainLogoDeep.height - 130;
			
			mainLogoNet.x = mainLogoDeep.x + 545;
			...
			barBack.y = stage.stageHeight - barBack.height - 60;
			...
			addChild(glasser);
			glasser.x = barBack.x + (barBack.width - glasser.width) / 2 -2;
			...
			var filter:GlowFilter = new GlowFilter(0x0b4056, 1, 3, 3, 6, 1);
			var shadowFilter:DropShadowFilter = new DropShadowFilter(2, 90, 0x0b4056, 1, 1, 1, 1, 1); 
		...
		{
			var _eyeCoords:Object = {x:mainLogoDeep.x + 415, y:mainLogoDeep.y + 90};
			var coords:Object = {};
		...
		private function drawSides():void {
			...
			/*var offsetLeft:int;
			var offsetRight:int;
			var color:uint = 0x082d48;*/
			/*if (stage.stageWidth < bg.width)
			{
				offsetLeft = -15;
				offsetRight = stage.stageWidth - 15;
			}
			else
			{
				offsetLeft = bg.x - 15
				offsetRight = bg.x + bg.width - 15;
			}
			var leftShape:Shape = new Shape();
			leftShape.graphics.beginFill(color, 1);
			leftShape.graphics.drawRect( offsetLeft, 0, 30, stage.stageHeight);
			leftShape.graphics.endFill();
			leftShape.filters = [new BlurFilter(30, 0)];
			addChild(leftShape);
			
			var rightShape:Shape = new Shape();
			rightShape.graphics.beginFill(color, 1);
			rightShape.graphics.drawRect( offsetRight, 0, 30, stage.stageHeight);
			rightShape.graphics.endFill();
			rightShape.filters = [new BlurFilter(30, 0)];
			addChild(rightShape);*/
			
			var widthMask:int = bg.width > stage.stageWidth? stage.stageWidth : bg.width - 60
			
			var bgMask:Shape = new Shape();
			bgMask.graphics.beginFill(0xff0000, 1);
			bgMask.graphics.drawRect(0, 0, widthMask, stage.stageHeight);
			bgMask.graphics.endFill();
			
			bgMask.x = bg.x + (bg.width - bgMask.width) / 2;
			bgMask.y = bg.y + (bg.height - bgMask.height) / 2;
			bgMask.cacheAsBitmap = true;
			bg.cacheAsBitmap = true;
			bgMask.filters = [new BlurFilter(60, 0)];
			addChild(bgMask);
			bg.mask = bgMask;
		}
