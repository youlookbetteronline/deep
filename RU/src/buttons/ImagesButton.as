package buttons 
{
	...
	public class ImagesButton extends ImageButton
	{
		...
		public function ImagesButton(bitmapData:BitmapData, icon:BitmapData = null, settings:Object = null, _scale:Number = 1):void 
		{
			...
			settings['building'] = settings.building || false;
		...
		override protected function drawBottomLayer():void {
			...
			{
			if (iconBmp.width > bitmap.width) {
				iconBmp.width = bitmap.width;
				iconBmp.scaleY = iconBmp.scaleX;
			}else {
				iconBmp.height = bitmap.height;
			...
			iconBmp.y = settings['iy'] || ((_bitmapData.height - iconBmp.height) / 2);
		}	
		...
		public function set icon(icon:BitmapData):void {
			...
			/*if (iconBmp.width > _bitmapData.width - 20) {
				iconBmp.scaleX = iconBmp.scaleY = (_bitmapData.width - 20)/(_bitmapData.width);
			}
			if (iconBmp.height > _bitmapData.height - 70 ) {
				iconBmp.height =  _bitmapData.height - 70;
				iconBmp.scaleX = iconBmp.scaleY;// = (background.height - 70) / (bitmap.height);
			}*/
		}
