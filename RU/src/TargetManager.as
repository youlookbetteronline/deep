package  
{
	...
	public class TargetManager
	{
		...
		public function dispose():void {
			...
			/*if (currentTarget.target.hasOwnProperty('model') && target.target.model.hasOwnPropert('reserved') && target.target.model.reserved > 0){
				currentTarget.target.model.reserved = 0;
			}*/
			currentTarget = null;
			status = FREE;
			for each(var target:* in queue) {
				...
				/*if (target.target.hasOwnProperty('model') && target.target.model.hasOwnPropert('reserved') && target.target.model.reserved > 0){
					target.target.model.reserved = 0;
				}*/
			}
			queue = [];
		}
