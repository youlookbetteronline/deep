...
package com.worlize.websocket
{
	...
	import wins.AuctionConnectionWindow;
	import wins.AuctionWindow;
	import wins.ConnectionWindow;
	...
	public class WebSocket extends EventDispatcher
	{
		...
		public function connect():void {
			if (_readyState === WebSocketState.INIT || _readyState === WebSocketState.CLOSED) {
				...
				Log.alert({title:'Connect', post:"Connecting to " + _host + " on port " + _port});
			}
		}
		...
		private function handleSocketClose(event:Event):void {
			...
			} else if (App.user.auction.windowOpened)
			{
		...
		private function processFrame(frame:WebSocketFrame):void {
			...
			switch (frame.opcode) {
				...
				case WebSocketOpcode.CONNECTION_CLOSE:
					...
					/*if (App.owner && App.owner.id == '1')
					{
						new ConnectionWindow({}).show();
						return;
					}*/
					if (!Window.hasType(ConnectionWindow) && !Window.hasType(VisitWindow) && App.user.mode == User.PUBLIC)
					{
						new ConnectionWindow({}).show();
						return;
					}
					if (App.user.auction.auctionWindowOpening || App.user.auction.windowOpened)
					{
						new AuctionConnectionWindow({}).show();
						return;
					}
		...
		private function handleSocketIOError(event:IOErrorEvent):void {
			...
			Log.alert({title:'handleSocketIOError', event:event});
			Cc.error(event.toString());
			dispatchEvent(event);
			dispatchClosedEvent();
		}
		...
		private function handleSocketSecurityError(event:SecurityErrorEvent):void {
			...
			//Log.alert({title:'handleSocketSecurityError', event:event});
			Cc.error(event.toString());
