package wins 
{
	...
	public class FlowerStandWindow extends Window
	{
		...
		public var backWindow:Shape = new Shape();
		
		public function FlowerStandWindow(settings:Object = null):void
		{
			if (settings == null) 
			{
				settings = new Object();
		...
		override public function drawBody():void 
		{
			...
			var ratios:Array = [100, 110];
			var bgColor:Array = [0x6ba9d4, 0xe8d496];
			
			var matrix:Matrix = new Matrix();
			var boxWidth:Number = settings.width - 80;
			var boxHeight:Number = settings.height - 80;
			var boxRotation:Number = Math.PI/2;


			matrix.createGradientBox(boxWidth, boxHeight, boxRotation);
			
			backWindow.graphics.beginGradientFill(gradType, bgColor, alphas, ratios, matrix);
			backWindow.graphics.drawRect(0, 0, settings.width - 90, settings.height - 100);
			backWindow.graphics.endFill();
			backWindow.x = (settings.width - backWindow.width) / 2 ;
			backWindow.y = (settings.height - backWindow.height) / 2 - 50;
			backWindow.filters = [new BlurFilter(10, 20, 2)];
			bodyContainer.addChild(backWindow); 
		...
		private var _materialsBG:Bitmap = new Bitmap();
		...
		{			
			...
			_houseBG = new Bitmap(Window.textures.flowerHouse);
			_houseBG.x = 25;
			_houseBG.y = -135;

			_acceptedMaterialsSprite.addChild(_houseBG);

			var _acceptedMaterialItems:Vector.<FlowerIcon> = new Vector.<FlowerIcon>();			
			
			var currentItem:FlowerIcon;
			var dX:int = 0;
			var iconSize:int = 100;
			var currentCharterIcon:FlowerIcon;
			var itemInfo:Object;
			var materialSid:int;
			...
			for (var i:int = 0; i < Numbers.countProps(bonusListObj); i++) 
			{
				...
				currentItem = new FlowerIcon(Numbers.getProp(bonusListObj, i).key, false, 0, iconSize, 20, null, true, materialSid, this);
			...
			if(!_acceptedMaterialsSprite.contains(materialsContainer)) {
				materialsContainer.x = 315;
				materialsContainer.y = (_acceptedMaterialsSprite.height - iconSize) * 0.5 + 15;
		...
		{			
			...
			var y:int = (settings.height - paginator.arrowLeft.height) / 2 + 105;
			...
			paginator.y = int(settings.height - paginator.height - 9);
		}
	}
}
...
import wins.Window;
	...
	{		
		...
		public function ApothecaryItem (item:Object, parent:*) {
			...
			bg = Window.backing(138, 165, 15, 'itemBacking');
			addChild(bg);
		...
		private function onBuy(e:MouseEvent):void 
		{
			...
			{
				Hints.text(Locale.__e('flash:1487256271248'), Hints.TEXT_RED, new Point(App.self.mouseX, App.self.mouseY));//Рыбкам не нужно это лекарство!
				return;
			}
			...
			Post.send({
				...
				for (var h:* in data.bonus) 
				{
					...
					//scount += App.data.storage[data.mID].cost;
