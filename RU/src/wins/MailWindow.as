package wins 
{
	...
	public class MailWindow extends Window
	{
		...
		public function drawTime():void {			
			...
			timerText.y = 75;
			timerText.x = (descriptionTimeLabel.width - timerText.width)/2;
			timeConteiner.addChild(timerText);
			
			var timerBacking:Bitmap = Window.backingShort(timerText.width + 100, 'backingGrad', true);
			timerBacking.alpha = .7;
			timerBacking.smoothing = true;
			timerBacking.x = timerText.x + (timerText.width - timerBacking.width) / 2 + 0;
			timerBacking.y = timerText.y + (timerText.textHeight - timerBacking.height) / 2 + 0;
			timeConteiner.addChild(timerBacking);
			timeConteiner.swapChildren(timerText, timerBacking);
			timeConteiner.addChild(descriptionTimeLabel);
			
			timeConteiner.x = (background.width - timeConteiner.width) / 2 + 0;//55 + dX;
			timeConteiner.y = 295;
