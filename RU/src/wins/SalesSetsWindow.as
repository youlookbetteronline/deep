package wins 
{
	...
	public class SalesSetsWindow extends Window
	{
		...
		public function SalesSetsWindow(settings:Object = null, _bulkSid:int = 1)
		{
			if (settings == null) {
				settings = new Object();
			}
		...
		override public function drawArrows():void {
			...
			paginator.arrowLeft.x = paginator.arrowLeft.width - 22;
		...
		{	
			...
			drawMirrowObjs('storageWoodenDec', -10, settings.width - 10, settings.height - 130);
			drawMirrowObjs('storageWoodenDec', -10, settings.width - 10, 30, false, false, false, 1, -1);
		...
		public override function contentChange():void 
		{
			...
			for (var i:int = paginator.startCount; i < paginator.finishCount; i++)
			{
				...
				if (App.data.bulkset.hasOwnProperty(itemSid)) 
				{
				var item:ActionItem = new ActionItem(bulkSid, App.data.bulkset[itemSid], itemSid);
...
internal class ActionItem extends Sprite {
		...
		public function ActionItem(_bulkSid:int, _item:Object, _itemSid:int ) {
			...
			this.item = _item;
			order = App.data.bulks[_bulkSid].iorder[_itemSid];
			for (var id:* in App.data.bulks[_bulkSid].items) {
				if (App.data.bulks[_bulkSid].items[id] == _itemSid) 
				{
					this.id = id;
				}
			}
			...
			rubyIcon = new Bitmap(UserInterface.textures.blueCristal, "auto", true);
		...
		{
			var rubyIcon:Bitmap = new Bitmap(UserInterface.textures.blueCristal, "auto", true);
		...
		{
			if (App.user.stock.data[Stock.FANT] < App.data.bulkset[sid].price_new) {
				App.user.stock.check( Stock.FANT,App.data.bulkset[sid].price_new)
				return
			}
