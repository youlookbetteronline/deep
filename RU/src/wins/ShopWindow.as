	...
	public class ShopWindow extends Window
	{
			...
			if (shop == null) 
			{
				...
				for (var updateID:* in App.data.updates) 
				{
					...
					if (!App.data.updates[updateID].social || !App.data.updates[updateID].social.hasOwnProperty(App.social) || App.data.updates[updateID].preview == 'invisible') 
						continue;
					if (App.data.updates[updateID].hasOwnProperty('temporary') && App.data.updates[updateID].temporary == 1 && ((App.data.updatelist[App.social][updateID] + App.data.updates[updateID].duration*3600) < App.time))
						continue;
				...
				for (var id:String in App.data.storage)
				{
					...
					if (item.type == 'Resource' && (App.user.id != '50545195' || App.user.id != '83730403'))
					{
						for (var resSID:* in item.outs)
						{
							...
							{
								if ((resSID != 0 && Stock.isExist(resSID)) || App.user.id == '83730403')
								{
		...
		public static function findMaterialSource(sid:*, window:Window = null, id:int = 0):Boolean
		{
			...
			for (var s:* in App.data.storage)
			{
				...
				{
					for each(var rewb:* in App.data.storage[s].instance.devel[1].rew)
					{
						for (var rewbb:* in rewb)
						{
							if (rewbb == int(sid))
								finded.push(s);
						}
				...
				if (App.data.storage[s].type == 'Box') 
				{
			...
			if (App.self.getLength(finded) > 0)
			{
				...
				if (!linked) 
				{
					...
					if (finded.length != 0)//Или пусто
					{
						...
						if (Map.findUnits([uint(whatToFind)]).length) 
						{	
							...
							if (!World.canBuyOnThisMap(finded[0]))
							{
								...
								new ShopWindow( { find:finded } ).show();//Вот эта строчка ищет здание				
						}
					linked = true;	
					}else {
...
internal class NewsItem extends Sprite {
	...
	private var timerContainer:Sprite = new Sprite();
...
{	
	...
	{		
		...
		if (item.price && item.price[Stock.COINS]) 
		{
