package wins 
{
	...
	public class InstanceWindow extends BuildingWindow
	{
		...
		public function InstanceWindow(settings:Object = null)
		{
			...
			if (App.user.personages.length>3) 
			{
			//	settings['itemsCount'] =App.user.personages.length;
			settings['hasPaginator'] = true;
			settings['title'] = '';
			settings['itemsOnPage'] = 3; 
		//	settings['buttonsCount'] = 2;
			...
			for (var res:* in settings.roomInfo.require) {
				if (!App.user.stock.check(res, settings.roomInfo.require[res]) || res == Techno.TECHNO) {
		...
		private function showPersonages():void {
			...
			var arrPers:Array = new Array;
			if (this.settings.roomInfo.id == 202) 
			{
				arrPers = App.user.aliens.concat(App.user.characters);
				
			} else 
			{
			//	arrPers = App.user.techno;
			
			}
			
		//	arrPers.push(arrPers[0]);
		//	arrPers.push(arrPers[1]); //////////////////////////////////////////////////////////////////////////////////////////
		...
		override public function contentChange():void {
			...
			for (i = paginator.startCount; i < paginator.finishCount; i++)
			{
				...
				//arrPers[itemNum].change(_arrCraft[i].fid, _arrCraft[i].lvl, isHelp);
				persItems[itemNum].visible = true;
				itemNum++;
			}
		...
		public function hidePersonage(sid:uint):void {
			for each(var persItem:PersonageItem in persItems) {
				if (persItem.sid == sid) {
						...
						mtrx.saturation(0.3);
						persItem.filters = [mtrx.filter];
						persItem.mouseChildren = false;
		...
		override public function dispose():void {
			...
			if (inviteBttn) {
				...
				var yb:Boolean = !App.isJapan();
				inviteBttn.visible = yb;
				inviteBttn.dispose();
			}
			if (bttnImproveTime) {
		...
		private var decorLeftUp:Bitmap;
		
		override public function drawBody():void 
		{
			fader.visible = false;
			if (!isWork) {
				bottomBg = backing(278, 318, 20, 'shopBacking');
				
				bottomBg2 = backing(278, 318, 20, 'shopBacking');
				bottomBg3 = backing(288, 278, 20, 'shopBacking')
					bottomBg.x = 30;
					bottomBg.y = 0;
					bottomBg3.x = 245;
					bottomBg3.y = 85;
				...
				//	decorLeftUp = (Window.textures.decorTopLiana);
				//	decorLeftUp = 
					bottomsContainer.addChild(bottomBg2);
					
					addMirrowObjs(bottomsContainer, 'decorTopLiana', bottomBg.width / 2 -8 , bottomBg3.x + bottomBg3.width+118, -3, true, true,false,1);
					addMirrowObjs(bottomsContainer, 'decorTopLiana', bottomBg.width / 2 -10 , bottomBg3.x + bottomBg3.width+120, 316, true, true,false,1,-1);
					addMirrowObjs(bottomsContainer, 'decorTopLiana', bottomBg2.x-120  , bottomBg2.x + bottomBg2.width-324, 100, true, true,false,1,-1);
					addMirrowObjs(bottomsContainer, 'decorBotHameleo', bottomBg3.x + bottomBg3.width+300  ,bottomBg.x -80, bottomBg3.height-110, true, true,false,1);
					
					
					bottomsContainer.addChild(bottomBg3);
					addMirrowObjs(bottomsContainer, 'decorBotFlower', bottomBg2.x-120  , bottomBg2.x + bottomBg2.width-324, 246, true, true,false,1,1);
				
					
					
				exit = new ImageButton(textures.closeBttn);
					bottomsContainer.addChild(exit);
					exit.x = bottomBg2.x + bottomBg2.width - 30;
					exit.y = bottomBg2.y -15;
					exit.addEventListener(MouseEvent.CLICK, close);
					
				addChild(bottomsContainer);
					bottomsContainer.y = App.self.stage.stageHeight - bottomBg3.height - 15;
		...
		public function drawOuts():void
		{
			...
			outsTitle.x = (bgWidth - outsTitle.textWidth) / 2 - 34;
			outsTitle.y = - outsTitle.textHeight / 2 -24;
			...
			/*var underBg:Bitmap = Window.backing2(224, 260, 45, 'snstSmallBackingTop', 'insttSmallBackingBot');
			//underBg.visible = false;
			var clt:ColorTransform = new ColorTransform(1.1, 1.1, 1.2);
			underBg.transform.colorTransform = clt;
			//underBg.alpha = 0.7;
			outsCont.addChildAt(underBg, 0);
			underBg.x = -32;
			underBg.y -= 5;
			//underBg.visible = false;
			underBgMini = Window.backing2(254, 105, 45, 'snstSmallBackingTop', 'insttSmallBackingBot');
			//underBgMini.visible = false;
		//	underBgMini.alpha = 0.7;
			bottomsContainer.addChildAt(underBgMini, 15);
			underBgMini.x = 262;
			underBgMini.y = 115;*/
			...
			//outsCont.y = 50;
			bottomsContainer.addChildAt(outsCont,11);
		...
		public function drawRequires():void
		{
			...
			btnnBuyReq = new MoneyButton( {
				...
				type:"green",
				radius:22,
				bgColor:[0xb4dcff, 0x5d8bf3],
				bevelColor:[0xbfd2fd, 0x376cdb],
...
//
		...
		{
			var neededRes:Object = clone(settings.roomInfo.require);
			//neededRes = ;
			if (neededRes.hasOwnProperty(Techno.TECHNO)) 
			{
			delete	neededRes[Techno.TECHNO] ;
			}
			
			if (App.user.stock.checkAll(neededRes)) {
		...
		private function onStartClick(e:MouseEvent):void 
		{
			if (btnnStartReq.mode == Button.DISABLED) return;
				...
				if ((settings.roomInfo.require.hasOwnProperty(Techno.TECHNO)) ) 
				{
					var countArr:Array = Techno.freeTechno();
					
				if (countArr.length<settings.roomInfo.require[Techno.TECHNO]) 
				{
			...
			for (var i:int = 0; i < settings.roomInfo.require[Techno.TECHNO]; i++) 
			{
				for (var j:int = 0; j < App.user.techno.length; j++) 
				{
					if (App.user.techno[j].workStatus == WorkerUnit.FREE) 
					{
					setHeroInto(App.user.techno[j])	
					App.user.techno[j].workStatus = WorkerUnit.BUSY;
					//App.user.techno[j].visible = false;
					//App.user.techno[j].targetObject = Missionhouse;
					arrHeroesSids[App.user.techno[j].id] = (Techno.TECHNO);
					break
					}
				}
		...
		public function drawFriendsInfo():void
		{
			...
			underBg.x = 512;
			underBg.y += 30;
			bottomsContainer.addChildAt(underBg,12);
			...
			iconCont = new ImagesButton(  Window.textures.friendsBttn,null, { 
				description		:"Облачко",
				params			:{ }
			});
			...
			inviteBttn.y = descFriends.y + descFriends.textHeight;
			inviteBttn.visible = !App.isJapan();
...
internal class OutItem extends Sprite {
	...
	public function updateProgress(value:int = 0):void
	{
		...
		if (settings.target.arrHeroesSids.length == 0 && settings.target.roomID != 202) 
		{
			var key2:int = Techno.TECHNO;
			if(App.data.storage[settings.id].personages[key2])
				percent += App.data.storage[settings.id].personages[key2];
		}
		if (percent > 100)
			percent = 100;
...
internal class PersonageItem extends LayerX 
{
	...
	{
		if (persEnabled == true) {
			stand = new Bitmap(Window.textures.instCharBacking);
		}else
		{
		...
		addChild(stand);
		stand.scaleX = stand.scaleY = 0.85;
	...
	public function add(sid:uint, hasPers:Boolean = true):void {
		...
		switch(App.data.storage[sid].preview) {
			case "boy":
				icon =  new ImageButton(Window.textures.charBoyIco);
				icon.y -= 5;
			break;
			case "girl":
			
			//this.filters = [mtrx.filter];
				icon =  new ImageButton(Window.textures.charGirlIco);
				icon.y -= 1;
				icon.x += 3;
			break;
			case "undeadWoman":
