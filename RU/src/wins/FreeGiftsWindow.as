	...
	public class FreeGiftsWindow extends Window
	{
		...
		public function drawDescription():void{
			...
			switch (settings.mode){
				...
				case ASK: descText = Locale.__e('flash:1493108232181'); break;
			}
			...
			descCounter = Window.drawText(String(" " + 1 + "/" + 3), {
					color:0xffdf34,
					borderColor:0x6e411e,
					fontSize:32,
					textAlign:"center"
			...
			descCounter.x = descLabel.x + descLabel.textWidth + 30;
		...
		private function addSlider():void
		{
			...
			capasitySlider.y = 33;
			txtTaked.x -= txtTaked.width + 6 - capasityBar.x;
			txtTaked.y = 29;
		...
		private function createBttns():void
		{
			...
			takeGiftsBttn.x = freeGiftsBttn.x + freeGiftsBttn.width + 15;
			takeGiftsBttn.y = -10;
			...
			askGiftsBttn.x = takeGiftsBttn.x + takeGiftsBttn.width + 15;
			askGiftsBttn.y = -10;
		...
		private var __availableRequests:int;
		private function get availableRequests():int
		{
			__availableRequests = 1;
			return __availableRequests;
		}
		...
		private function drawAvailableRequests():void{
			...
			requestsCounter = Window.drawText(String(availableRequests) + "/" + String(totalRequests), {
				color:			0xf5ff57,
				borderColor:	0x7d3400,
				width:			100,
				fontSize:		20,
...
internal class FreeGiftItem extends Sprite
{
	...
	private function onGiftBttn(e:MouseEvent):void
	{
		...
		if (_mode == FreeGiftsWindow.FREE){
			...
			new AskWindow(AskWindow.MODE_ASK,  
			{ 	
				sID:		this.sID, 
	...
	{
		leftLabelTimer.text = TimeConverter.timeToCuts(1492813342 - App.time, true);
	}
	private function drawTimer():void
	{
		...
		leftLabelTimer = Window.drawText(TimeConverter.timeToCuts(1492813342 - App.time, true), {
				color:			0xf5ff57,
				borderColor:	0x7d3400,
				width:			100,
				fontSize:		20,
