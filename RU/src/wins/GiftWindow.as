package wins 
{
	...
	public class GiftWindow extends Window
	{
		...
		public function createPaginators():void {
			...
			paginatorDown.y = settings.height - paginatorDown.height + 7;
...
internal class Icon extends Sprite
{
	...
	private function drawCounter():void
	{
		...
		var colors:Array = [0xfbecaf, 0xedd994]; 
		var alphas:Array = [1, 1]; 
		var ratios:Array = [0, 255]; 
		var matrix:Matrix = new Matrix(); 
		var boxWidth:Number = 51; 
		var boxHeight:Number = 33; 
		var boxRotation:Number = Math.PI/2; // 90° 
		var tx:Number = 0; 
		var ty:Number = 0; 
		matrix.createGradientBox(boxWidth, boxHeight, boxRotation, tx, ty); 
		var countBg:Shape = new Shape();
		countBg.graphics.beginGradientFill(type, colors, alphas, ratios, matrix);
		countBg.graphics.drawRoundRect(0, 0, 51, 33, 10, 10);
		countBg.graphics.endFill();
		countBg.x = -countBg.width / 2 + 2;
		countBg.y = -29;
		countBg.filters = [new DropShadowFilter(1, 90, 0xab9a79, 0.7, 4, 4, 4, 1, true), new DropShadowFilter(1, -90, 0xfff9d9, .7, 4, 4, 4, 1, true)];	
