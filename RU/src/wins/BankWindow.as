package wins 
{
	...
	public class BankWindow extends Window
	{
		...
		override public function drawBackground():void {
			...
			var background:Bitmap = backing(settings.width, settings.height, 40, "BanksBackingItemBest");
			//var backing:Bitmap = Window.backing(590, 450, 15, "shopDarkBacking");
			//drawMirrowObjs('storageWoodenDec', -5, settings.width + 5, -37);
			//drawMirrowObjs('decorBotFlower', -5, settings.width + 5, settings.height - 90);
...
internal class BankItem extends Sprite{
	...
	public function BankItem(type:String, obj:Object, id:int)
	{
		...
		if (App.social == 'YB' && type == 'coins') {
			var fantsIcon:Bitmap = new Bitmap(UserInterface.textures.blueCristal);
...
internal class SetItem extends Sprite{
	...
	public function SetItem(item:Object, window:*, i:int) {
		...
		background = Window.backing(296, 130, 10, "banksBackingItemBest");
		addChild(background);
	...
	public function drawPrice():void {
		...
		var priceLable:Object = ActionsHelper.priceLable(item.price[App.social]);
		bttnSettings.caption = Locale.__e("flash:1382952379984") + ' ' + Locale.__e(priceLable.text, priceLable.price);
