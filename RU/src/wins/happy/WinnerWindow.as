package wins.happy 
{
	...
	public class WinnerWindow extends Window
	{
		...
		public function WinnerWindow (settings:Object = null) {
			...
			if (isWinner() )
				settings['title'] = Locale.__e ('flash:1472454762180');
			else 
				settings['title'] = Locale.__e('flash:1472455168196');
			super(settings);
		...
		protected function drawDesc():void {
			...
			if (isWinner() )
				descLabelText = Locale.__e( 'flash:1472455092916');
			else
				descLabelText = Locale.__e( 'flash:1472455209437');
			...
			Load.loading(Config.getImage("Thappy", settings.view), function(data:Bitmap):void {
				teamFlag.bitmapData = data.bitmapData;
				teamFlag.x = (settings.width - teamFlag.width) / 2 - 10;
				teamFlag.y = -teamFlag.height / 3-20;
				teamFlag.smoothing = true;
				
				descLabel.x = 50;
				descLabel.y = teamFlag.y + teamFlag.height;
			});
		}
