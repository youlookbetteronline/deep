package wins 
{
	...
	public class JamWindow extends Window
	{
		...
		private function initContent(view:String):Array
		{
			var item:*
			var result:Array = [];
			for (var i:int = 0; i < JamWindow.jams.length; i++)
			{
				if (App.data.storage[JamWindow.jams[i]].view.indexOf(view) != -1) {
					item = App.data.storage[JamWindow.jams[i]];
					item['sID'] = JamWindow.jams[i];
					item['glow'] = false;
					result.push(item);
				}
			}
...
internal class JamItem extends Sprite {
	...
	public function JamItem(item:*, window:*, id:*) {
		...
		jamTick.tip = function():Object { 
			return {
				title:"",
				text:Locale.__e("flash:1382952379767 âàðåíüÿ ïåðåïîëíåí.")
			};
		};
	...
	private function drawCapacity():void
	{
		...
		var countText:TextField = Window.drawText(String(item.capacity), textSettings);
