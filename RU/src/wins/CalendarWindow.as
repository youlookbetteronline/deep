	...
	{		
		...
		{			
			...
			if (App.data.options.hasOwnProperty('CalendarOpen')) {
				...
				if (!info.hasOwnProperty(App.social) /*|| info.social.indexOf(App.social) < 0*/) return;
				if (App.user.calendar.hasOwnProperty(Numbers.firstProp(info[App.social]).key) && App.user.calendar[Numbers.firstProp(info[App.social]).key].hasOwnProperty(Numbers.firstProp(Numbers.firstProp(info[App.social]).val).key))
				{
					for (var openYear:String in info[App.social]) {
						for (var openMonth:String in info[App.social][openYear]) {
							for (var i:int = 0; i < info[App.social][openYear][openMonth].length; i++) {
								var skip:Boolean = false;
								try {
									if ((App.user.calendar[openYear][openMonth].indexOf(info[App.social][openYear][openMonth][i]) >= 0)||(App.user.calendar[openYear][openMonth][0] == openYear)) {
										skip = true;
									}
								}catch (e:*) { }
								...
								Post.send( {
									...
									ctr:	'Calendar',
									act:	'take',
									d:		info[App.social][openYear][openMonth][i],
									m:		openMonth,
									y:		openYear
								...
								}, { day:info[App.social][openYear][openMonth][i] } );
							}
						}
					}
				}
		...
		override public function drawBody():void 
		{
			...
			//bodyContainer.addChild(bttnSave);
			bttnSave.x = 120;
			bttnSave.y = 575;
			bttnSave.addEventListener(MouseEvent.CLICK, onSaveEvent);
		...
		private function onSaveEvent(e:MouseEvent):void {
			Saver.saveJPG(calendarImage.bitmapData, 'calendar');
		}
...
internal class DayIcon extends LayerX {
	...
	public function onTake(e:MouseEvent = null):void
	{
		...
		TweenLite.to(bttnTake, 1, { alpha:0, onComplete:onTakeCallback});
	}
	
	private function onTakeCallback():void 
	{
