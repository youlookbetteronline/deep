	...
	public class ShopItem extends LayerX {
		...
		public function ShopItem(item:*, window:*) {
			...
			if (item.type == 'Plant') 
			{
				...
				if ((/*item.type == 'Decor' ||*/ item.type == 'Golden' || item.type == 'Walkgolden'|| item.type == 'Banker' || item.type == 'Animal'|| item.type == 'Mfloors'|| item.type == 'Beast'))
				{
		...
		private function load():void {
			...
			if (item.type == 'Decor' || item.type == 'Golden' || item.type == 'Walkgolden' || item.type == 'Beast')
		...
		private function drawButton():void 
		{
			...
			if (item.type == 'Golden' || item.type == 'Tribute' || item.type == 'Gamble' || item.type == 'Walkgolden')
				setStripe();
			...
			if ((item.type == 'Port' || item.type == 'Barter') && item.sid != 2315)	//Костыль для ГД Тони
				drawFindOnMapButton();
			else
				drawPriceBttn();
		...
		private function get levelLock():Boolean {
			...
			if (item.type == 'Plant')
			{
				var fields:Array = Map.findFieldUnits([76, 2198]);
				var countPlant:int = 0;
			...
			{
				var cinstances:Array;
				cinstances = Map.findUnits([item.sID]);
				if (cinstances.length >= aveableCount)
				{
					return true;
				}else 
					return false;
			}
			if (item.type == 'Bathyscaphe')
			{
				var batinstances:Array;
				batinstances = Map.findUnits([item.sID]);
				if (batinstances.length >= aveableCount)
				{
					return true;
				}else 
					return false;
			}
			if (item.type == 'Manufacture')
			{
				var maninstances:Array;
				maninstances = Map.findUnits([item.sID]);
				if (maninstances.length >= aveableCount)
				{
					return true;
				}else 
					return false;
			...
			{
				var univinstances:Array;
				univinstances = Map.findUnits([item.sID]);
				if (univinstances.length >= aveableCount)
				{
					return true;
				}else 
					return false;
			...
			if (item.type == 'Barter')
			{
				var binstances:Array;
				binstances = Map.findUnits([item.sID]);
				if (binstances.length >= aveableCount)
				{
					return true;
				}else 
					return false;
			...
			if (item.type == 'Picker')
			{
				var pinstances:Array;
				pinstances = Map.findUnits([item.sID]);
				if (pinstances.length >= aveableCount)
				{
					return true;
				}else 
					return false;
			...
			if (item.type == 'Walkhero')
			{
				var wheroInst:Array = Map.findUnits([item.sID]);
				
				if (wheroInst.length >= aveableCount)
					return true;
			}
			
			if (item.type == 'Walkgolden')
			{
				var wgoldInst:Array = Map.findUnits([item.sID]);
				
				if (wgoldInst.length >= aveableCount)
					return true;
			}
			
			if (item.type == 'Animal' && item.gcount)
			{
				var animInst:Array = Map.findUnits([item.sID]);
				
				if (animInst.length >= aveableCount)
					return true;
			}
			...
			if (item.type == 'Port' || item.type == 'Barter'|| item.type == 'Portal')
			{
				//var instns:Array;
		...
		public function get aveableCount():int
		{
			...
			item.type == 'Walkhero' || item.type == 'Craftfloors' || item.type == 'Bathyscaphe' || item.type == 'Animal') 
		...
		private function ableOnMap():Boolean 
		{
			...
			if (item.type == 'Picker')
			{
				var pickerInst:Array = Map.findUnits([item.sID]);
				
				if (pickerInst.length >= aveableCount)
					return false;
			}
			...
			if (item.type == 'Walkhero')
			{
				...
				if (instances.length >= aveableCount)
				{
					drawText(Locale.__e('flash:1513780744455')); // Нигде не доступно
					return false;
				}
			...
			if (item.type == 'Animal' && item.gcount)
			{
				...
				if (instances.length >= aveableCount)
				{
					drawText(item.title + ' ' + instances.length + '/' + aveableCount) // Нигде не доступно
					return false;
				}
			...
			/*if (item.type == 'Plant')
			{
				...
				if (instances.length >= aveableCount)
				{
					drawText(Locale.__e('flash:1510647870090') + ' ' + String(instances.length) + '/' + String(item.gcount)); // Нигде не доступно
					return false;
				}*/
		...
		private function drawFindButton():void 
		{
			...
			findButton.x = (background.width - findButton.width) / 2;
			findButton.y = background.height - findButton.height + 15;
		...
		{
			Window.closeAll();
			if (App.data.storage[this.item.sID].hasOwnProperty('available') && App.data.storage[this.item.sID].available != '' && !TravelWindow.availableByTime(this.item.sID))
			{	
		...
		{
			if (item.sid == 2129 || item.sid == 2587 || item.sid == 3123 || item.sid == 2990)
				return;
			//setAlpha();
			var bttnSettings:Object = {
				caption:Locale.__e("flash:1407231372860"),
		...
		private function onFindEvent(e:MouseEvent):void 
		{
			...
			if (item.sid == 2981)
			{
				if (World.worldsWhereEnabled(item.sid).indexOf(App.user.worldID) != -1)
				{
					if (!World.zoneIsOpen(App.map._aStarNodes[onMap[0]['coords']['x']][onMap[0]['coords']['z']].z))
					{
						new SimpleWindow( {
							title:Locale.__e("flash:1474469531767"),
							label:SimpleWindow.ATTENTION,
							text:Locale.__e('flash:1481876277712', item.title),
							popup:true
						}).show();
						return;
			...
			if (item.sid == 2691)	// поиск на улитку ВГ
			{
				if (onMap.length > 0)
				{
				...
				{
					Locker.availableWith(2697) // можно получить с улитки-банкира
					return;
				}
			}
			if (item.sid == 2590)	// поиск на улитку ВГ
			{
				if (onMap.length > 0)
				{
					App.user.quests.findTarget([onMap[0].sid], false, null, false, onMap[0].sid, false);
				...
				{
					Locker.availableWith(2830) // можно получить с витража
					return;
				}
			}
			if (onMap.length > 0) 
			...
			//747
			else if (item && item.sid && item.sid == 2582)
			{
				if (App.user.worldID == User.HOME_WORLD)
					Finder.questInUser(747, true);
				else
				{
					new WorldsWindow( {
						title:		 Locale.__e('flash:1415791943192'),
				...
				var itemIsAweable:Array = ShopMenu.getMapShop(App.user.worldID);
				if (!World.canBuyOnThisMap(item.sid) || itemIsAweable.indexOf(item.market) == -1)
				{
					if(item.market == 14){
						new WorldsWindow( {
							title: Locale.__e('flash:1415791943192'),
							sID:	item.sid,
							only:	World.worldsWhereEnabledTry(item.sid),
							popup:	true
						}).show();
					}else{
						new WorldsWindow( {
					...
					Window.closeAll();
					new ShopWindow({find:[item.sID]}).show();
				}
				/*new SimpleWindow( {
		...
		public function drawPriceBttn():void {
			...
			if (item.type == 'Collection' || item.type == 'Portal' || item.type == 'Material' || (App.user.worldID == User.SCIENCE_MAP && window.settings.section == 101))
				return;
