...
internal class QueueItem extends Sprite 
{
	...
	public function QueueItem(_items:Object, _mode:Boolean, _numberInQueue:int, _sid:int, _id:int, _type:String, _openedSlots:int, _target:Object, _craftingTime:Array, _productionQueue:ProductionQueue) 
	{
		...
		sprTip.tip = function():Object {
				return {
					...
					text:Locale.__e(Locale.__e('flash:1409663405135') +  '\n' + TimeConverter.timeToStr(target.crafted-App.time + craftingTime)),
					timer:true
				}
			}
		for (var item:* in App.data.storage[sid].devel.craft[1][numberInQueue +1])//for (var item:* in App.data.storage[sid].slots[numberInQueue +1].req)
		{
	...
	{
		backCont = new Sprite();
		back = new Bitmap(Window.textures.craftSlot);
		back.alpha = 0.7;
	...
	{
		text = Window.drawText(Locale.__e('flash:1409665986959'), {
			color		:0xe1d09d,
			borderSize	:0,
			widht		:80,
			wrap		:true,
	...
	{
		lock = new Bitmap(Window.textures.lockedSlot);
