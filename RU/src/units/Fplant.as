package units 
{
	...
	public class Fplant extends AUnit
	{
			...
			tip = function():Object {
				if(started != 0){
					if (started > App.time) {
						return {
							title:info.title,
							text:Locale.__e('flash:1396606641768'),
			...
			if (!App.user.stock.takeAll(App.data.storage[sid].require)) {
				for (var req:* in App.data.storage[sid].require) {
					...
					new PurchaseWindow( {
						width:382,
						itemsOnPage:2,
						content:PurchaseWindow.createContent("Energy", { inguest:0, view:'Water'} ),
						find:req,
						shortWindow:true,
						closeAfterBuy:false,
						autoClose:false,
		...
		private function flyMaterial(sid:int):void
		{
			...
			var point:Point = new Point(10, App.self.stage.stageHeight - 30);//Window.localToGlobal(App.ui.bottomPanel.bttnMainStock);
