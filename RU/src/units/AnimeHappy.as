package units 
{
	...
	public class AnimeHappy extends LayerX 
	{
		...
		public function AnimeHappy(swf:*, params:Object = null) 
		{
			if (params) {
		...
		public function draw():void {
			bounds = AnimeHappy.bounds(textures, ((params.animal) ? params : null));
			...
			if (texture.hasOwnProperty('animation') ) {
				for (var anims:String in texture.animation.animations) {
					...
					for (var _frame:String in texture.animation.animations[anims].frames) {
						var frame:Object = (obj.animal) ? texture.animation.animations[anims].frames[_frame][0] : texture.animation.animations[anims].frames[_frame];
