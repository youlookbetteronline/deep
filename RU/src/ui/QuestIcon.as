package ui
{
	...
	public class QuestIcon extends LayerX
	{
		...
		public var backingDecor:Bitmap;
		...
		public function drawIcon():void 
		{
			...
			Load.loading(Config.getImageIcon('quests/icons', character), function(data:Bitmap):void 
			{
				...
				iconTimer.x = bg.x -2;
				iconTimer.y = bg.y + 55;
				if (App.data.quests[qID].update && App.data.updates[App.data.quests[qID].update].hasOwnProperty('temporary') && App.data.updates[App.data.quests[qID].update]['temporary'] > 0)
					addChild(iconTimer);
