package
{
	...
	{	
								...
								if (unit.sid == sID)
								{
									...
									switch(missions[mID].event) {
										...
										case 'instance':
											...
											break;
										case 'finished':
											if (['Building','Booster','Hippodrome','Twigwam','Port', 'Pharmacy', 'Barter', 'Thappy', 'Field', 'Factory', 'Mining', 'Moneyhouse','Fatman', 'Golden', 'Storehouse','Tribute','Floors','Oracle', 'Mfloors', 'Table'].indexOf(unit.type) != -1) {
												if(scored[quest.id] == undefined){
													scored[quest.id] = { };
												}
										...
										case 'reward':
											...
											{
												if (unit['model'].level >= missions[mID].need) 
												{
													if(scored[quest.id] == undefined){
														scored[quest.id] = { };
													}
		...
		switch(event) 
		{
				...
				case 1:
					...
					}else 
					{
						...
						if (App.data[questType][qID].missions[mID].event == 'remove' && App.data[questType][qID].missions[mID].controller != 'mfloors' && (qID != 426) && !App.data.quests[qID].tutorial && App.data.storage[int(target)].type != 'Resource') //удаление
						{
							App.ui.bottomPanel.bttnCursor.showGlowing();
							App.ui.bottomPanel.bttnCursor.dispatchEvent(new MouseEvent(MouseEvent.CLICK));
		...
		public function findTarget(sIDs:Array, searchByType:Boolean, filter:Object = null, all:Boolean = false, find:int = 0 , light:Boolean = false, eventType:String = '', exception:Boolean = false):Boolean 
		{
			...
			{
				if (sIDs.length > 0 && (find == 1388 || find == 2404))
				{
					...
					}else
					{
						while (childs--) 
						{
							...
							if (unit is Unit && unit.sid == sID) 
							{
								...
								if (!(unit is Animal) && eventType == 'ready' && unit.hasOwnProperty('totalLevels') && (unit.level < unit.totalLevels)) 
								{
									continue;
								}
								if (unit is Plant)
